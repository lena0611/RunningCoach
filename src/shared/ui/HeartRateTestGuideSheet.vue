...
<template>
  <Teleport to="body">
    ...
    <div v-if="open" class="bottom-sheet-layer" role="presentation" @click.self="emit('close')">
      <section ref="sheetEl" tabindex="-1" class="bottom-sheet scheduling-help-sheet" :class="{ 'bottom-sheet-dragging': drag.dragging.value }" :style="drag.sheetStyle.value" role="dialog" aria-modal="true" aria-label="역치심박 측정 가이드">
