...
import { computeTempoCeilingAdaptation, gradeTempoRun, MAX_TOTAL_RAISE_BPM } from './tempoAdaptation'
...
describe('computeTempoCeilingAdaptation — 다단계 래칫(영속 채택값 재앵커링)', () => {
  ...
  it('RPE 없는 자동 import 런도 객관 신호(초과·후반 안정·회복) 충족 시 검증된다(완화)', () => {
    const runs = qualifyingSet(3, 165).map((r) => (r.type === 'Tempo' ? { ...r, rpe: null } : r))
    const a = computeTempoCeilingAdaptation(runs, BASE)
    expect(a.qualifyingCount).toBe(3)
    expect(a.effectiveCeilingBpm).toBe(BASE + 2)
  })

  it('적응 상한은 base + 안전망(MAX_TOTAL_RAISE_BPM)을 넘지 않는다(폭주·변조 방지)', () => {
    const a = computeTempoCeilingAdaptation([], BASE, { adoptedCeilingBpm: BASE + 100 })
    expect(a.effectiveCeilingBpm).toBe(BASE + MAX_TOTAL_RAISE_BPM)
  })

  it('회복 런이 회복 창(5일)을 벗어나면 검증되지 않는다', () => {
    const tempo = makeRun({ id: 't', type: 'Tempo', date: '2026-06-10', maxHeartRate: 165, rpe: 6 })
    const farNext = makeRun({ id: 'n', type: 'Easy', date: '2026-06-20', maxHeartRate: 130, rpe: 3 }) // 10일 뒤
    const a = computeTempoCeilingAdaptation([tempo, farNext], BASE)
    expect(a.qualifyingCount).toBe(0)
  })
})

describe('computeTempoCeilingAdaptation — 노이즈 방어(완화 후 false-positive 차단)', () => {
  // 평균심박이 낮은(유지되지 않은) 런: maxHr가 상한을 1~2bpm 스쳐도 검증 아님.
  function blipSet(count: number): RunLog[] {
    const runs: RunLog[] = []
    for (let i = 0; i < count; i++) {
      const day = 10 + i * 4
      runs.push(makeRun({ id: `blip-${i}`, type: 'Tempo', date: `2026-06-${String(day).padStart(2, '0')}`, maxHeartRate: BASE + 2, avgHeartRate: 128, rpe: null }))
      runs.push(makeRun({ id: `e-${i}`, type: 'Easy', date: `2026-06-${String(day + 1).padStart(2, '0')}`, maxHeartRate: 130, conditionScore: 4 }))
    }
    return runs
  }

  it('평균심박이 낮은 1bpm blip(이지런 오분류)은 지속 자극 게이트로 배제', () => {
    const a = computeTempoCeilingAdaptation(blipSet(3), BASE)
    expect(a.qualifyingCount).toBe(0)
    expect(a.effectiveCeilingBpm).toBe(BASE)
  })

  it('단일 HR 스파이크(상한 +10 초과)는 등급 C로 검증 제외', () => {
    const runs = qualifyingSet(3, 165).map((r, i) =>
      r.type === 'Tempo' && i === 0 ? { ...r, maxHeartRate: 210 } : r
    )
    const a = computeTempoCeilingAdaptation(runs, BASE)
    // 스파이크 1건은 C로 빠지고 나머지 2건만 검증 → 후보(관찰)에 그침, 자동 적용 안 함.
    expect(a.qualifyingCount).toBe(2)
    expect(a.effectiveCeilingBpm).toBe(BASE)
  })
})
