...
describe('기억 탭 draft 병합', () => {
  it('로드 전 빈 draft 는 도착한 값으로 채워진다 — 다른 칸을 편집 중이어도', () => {
    const draft: Memory = { aiNotes: [], knownIssues: [], goal: '수정 중' }
    const snapshot: Memory = { aiNotes: [], knownIssues: [], goal: '' }
    const fresh: Memory = { aiNotes: ['a'], knownIssues: ['b'], goal: '서버 목표' }

    mergeStoreIntoDraft(draft, snapshot, fresh)

    expect(draft.aiNotes).toEqual(['a'])
    expect(draft.knownIssues).toEqual(['b'])
