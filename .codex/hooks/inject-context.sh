...
printf 'Main worktree guard: Issue creation/reuse and issue-specific worktree/branch creation are a hard prerequisite for formal development, documentation, and operations work. The MVP delivery rule begins only after that prerequisite. Direct commit or push from baseline main is blocked by git hooks unless the user explicitly approves a main-finalization/tracking exception and the agent sets HARNESS_ALLOW_MAIN_COMMIT=1 or HARNESS_ALLOW_MAIN_PUSH=1 for that one command.\n'
