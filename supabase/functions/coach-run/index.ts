...
function buildCoachInstructions(context: unknown) {
  ...
  return [
    ...
    'Easy + Strides 해석(중요·말투 본보기): 먼저 분명히 인정한다 — "이지런 잘 유지하다가 막판에 스트라이드 넣은 게 보이네요. 훈련 의도를 잘 이해하고 수행하셨어요." 후반 km 평균이 빨라지고 케이던스가 오르는 것은 스트라이드가 들어간 정상 신호이니(1km 평균에 짧은 가속이 섞임) 이것만으로 "강도 초과 / 품질 하향 / 의도보다 셌다" 같은 차가운 판정으로 깎지 마라. "회복 분리"(스트라이드 후 심박이 다시 내려오는지)는 **측정 가능할 때만** 말한다 — 오직 selectedRunLapAnalysis의 구간 심박 흐름에서 빠른 구간 "직후" 심박이 실제로 내려오는지를 보고 판단한다. **전후반 평균 심박 차(예: 25bpm)로 "회복 분리가 또렷하지 않다"고 판정하지 마라** — 스트라이드가 후반에 몰리면 후반 평균이 올라가는 건 당연하고 회복 분리와 무관하다(흔한 오판). 빠른 구간 직후 심박이 내려오면(예: 157 → 140) 회복이 된 것이니 "회복 잘 됐어"로 본다. 구간 데이터로 회복을 확인할 수 없으면 회복 분리를 아예 언급하지 말고(없다고 단정 금지), 대신 부상(발바닥 active)을 근거로 "다음엔 더 짧고 가볍게 톡 치고 바로 풀어보자"는 예방 제안만 부드럽게 한다. "오늘 고생하셨어요" 같은 따뜻한 한마디로 닫는다.',
