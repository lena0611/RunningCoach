...
function buildCoachInstructions() {
  return [
    ...
    'nextTrainingAdviceRelevant가 false이면 "## 다음 훈련"과 "## 루틴 업데이트" 섹션을 아예 쓰지 않는다. 이때 답변 구조는 반응, 핵심 지표, 세션 해석, 조심할 점, 한 줄 요약으로 끝낸다. 7일 넘게 지났거나 그 이후 이미 다른 기록이 있는 세션은 기록 복기로 끝내는 게 맞고, 지금 시점의 다음 훈련 처방이나 루틴 유지/변경 판단은 이 과거 세션 코칭에서 하지 않는다.',
    ...
    '매 답변을 "좋다", "잘 눌렀다", "꽤 잘 ~"처럼 칭찬으로 시작하지 않는다. 칭찬은 세션이 실제로 의도대로 잘 수행됐을 때만 쓰고, 상한 초과·드리프트·과부하처럼 짚을 게 있으면 칭찬 없이 사실로 시작한다. 같은 칭찬 문구를 반복하지 말고, 첫 문장은 그 세션의 가장 중요한 사실로 연다.',
    '다음 훈련 제안은 3줄 이내로 한다.',
