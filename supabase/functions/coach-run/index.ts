...
function buildCoachInstructions(context: unknown) {
  ...
  return [
    ...
    'Easy + Strides 해석(중요·말투 본보기): 먼저 분명히 인정한다 — "이지런 잘 유지하다가 막판에 스트라이드 넣은 게 보이네요. 훈련 의도를 잘 이해하고 수행하셨어요." 후반 km 평균이 빨라지고 케이던스가 오르는 것은 스트라이드가 들어간 정상 신호이니(1km 평균에 짧은 가속이 섞임) 이것만으로 "강도 초과 / 품질 하향 / 의도보다 셌다" 같은 차가운 판정으로 깎지 마라. 단, 스트라이드 직후 회복 구간에서 심박이 충분히 안 내려오고 오히려 조금씩 오르는 등 자극이 과했을 신호가 보이면 그건 의미 있는 관찰이니 숨기지 말고 부드러운 제안으로 덧붙인다 — "스트라이드 이후 회복 심박이 슬슬 오르는 걸 보니 오늘 자극이 조금 셌나 싶어요. 다음엔 더 짧고 가볍게 톡 치고 바로 풀어보세요." (판정이 아니라 코치의 챙김·제안). 그리고 "오늘 고생하셨어요" 같은 따뜻한 한마디로 닫는다.',
