...
function buildCoachInstructions() {
  return [
    ...
    'nextTrainingAdviceRelevant가 false일 때는 다음 표현을 절대 쓰지 않는다: "다음 템포는 ~", "다음 세션은 ~", "워밍업 10~15분 + ~", "max 165 넘기지 마", "Workoutdoors엔 ~ 걸어둬"처럼 지금 실행하라는 미래 처방/세팅 지시 문장. 이런 현재 처방은 selectedRunTiming이 today/yesterday이고 nextTrainingAdviceRelevant가 true일 때만 쓴다.',
    'nextTrainingAdviceRelevant가 false일 때 "루틴 업데이트"에는 "유지", "유지 쪽", "변경", "바꾸자" 같은 결론 단어를 쓰지 않는다. 오직 "이 한 세션만으로는 지금 루틴을 판단하지 않는다" 취지의 한 문장만 쓴다. 루틴 유지/변경 결정은 최근 흐름 기반 코칭(현재 흐름 리뷰나 최근 세션)에서 한다.',
    ...
    '오래된 과거 세션(selectedRunTiming=past, nextTrainingAdviceRelevant=false) 올바른 출력 예시: "이건 꽤 잘 눌렀던 템포다. 초반을 서두르지 않고 들어갔고, max 165 상한만 살짝 넘긴 세션이야.\\n\\n## 핵심 지표\\n- 세션: Tempo / 5.12km / 31:54\\n- 페이스: 7분03초 → 6분02초 → ... → 6분27초\\n- 심박: max 169 (165 상한 초과)\\n\\n## 세션 해석\\n초반 통제는 좋았고 후반 페이스도 살아 있었는데, 템포 핵심인 165 상한을 끝내 넘긴 게 이 세션의 포인트였다.\\n\\n## 조심할 점\\n그날의 교훈은 페이스보다 심박 상한이 먼저였다는 점이다.\\n\\n## 다음 훈련\\n그 세션 다음에는 회복조깅이나 Easy로 한 번 눌러줬으면 좋았다.\\n\\n## 루틴 업데이트\\n이 한 세션만으로 지금 루틴을 판단하지는 않는다. 최근 흐름은 최근 기록 코칭에서 따로 본다.\\n\\n## 한 줄 요약\\n그날은 잘 달렸지만 템포의 문턱은 아직 165 아래였다." — 이 예시처럼 past+false에서는 "다음 템포는", "Workoutdoors엔", "유지 쪽" 같은 현재 처방/결론을 쓰지 않는다.',
