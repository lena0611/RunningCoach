  ...
  return [
    { role: 'system', content: buildCoachInstructions(context) },
    {
      ...
      content:
        ...
        // 실행 지침 꼬리표 **뒤에** 사용자 요청을 한 번 더 세운다. 그러지 않으면 프롬프트의 마지막
        // 블록이 예정 세션 목록이라, 대상이 생략된 짧은 요청("표로 보여줘")이 스케줄 비교로 끌려간다
        // (2026-09-16 실사고 2회 재현). 자세한 경위는 turnRequestTail.ts 주석.
        buildTurnRequestTail((context as { userNote?: unknown } | null)?.userNote)
    }
  ]
}
 ...
 */
/**
 * 코치의 **말투 계약** — 내부 용어·어색한 은유·가독성·구체성 (2026-09-16 사용자 지적).
 *
 * 실제로 새어 나온 문장들:
 *   "우측 족저근막이 아직 monitoring이라" — 내부 상태값을 그대로 읽었다.
 *   "러닝으로 밀어붙이기보다 조용한지 먼저 보는 게 맞습니다"
 *   "발바닥이 조용하게 이어질 때만" · "발이 조용한지 확인하는 날이에요"
 *   → 통증을 "조용하다", 훈련을 "밀다"로 말하는 건 우리말로 어색하다.
 *
 * 상태값은 **앱 화면 라벨을 그대로 미러한다**(MemoryPage·CoachSessionOverlay: 관리 중/관찰 중/해소).
 * 코치가 화면과 다른 말을 쓰면 사용자는 같은 것을 가리키는지조차 알 수 없다.
 *
 * 마지막 줄(구체성)이 이 앱의 경쟁력이다 — 범용 챗봇과 다른 점은 이 코치가 **사용자의 기록과
 * 지난 대화를 실제로 들고 있다**는 것뿐이다. 다만 일반 개념 질문까지 개인화하면 #559·#701 이
 * 막아둔 반대 방향 회귀(묻지 않은 세션 지표를 들이미는 것)라, userNoteRelevancePolicy 를 우선한다.
 */
function buildInternalNamingGuard() {
  return [
    ...
    '⚠️ **내부 상태값(영문 enum)도 그대로 말하지 마라.** 앱 화면에 쓰는 우리말로 옮긴다 — 부상 상태는 active=관리 중, monitoring=관찰 중, resolved=해소다. 세션 판정 토큰(met_/missed_), 품질 등급(A/B/C/D), canIntensify·redFlag·flare 같은 내부 값도 마찬가지다. "아직 monitoring이라"가 아니라 "아직 관찰 중이라"로 말한다.',
    '**어색한 은유를 쓰지 마라.** ① **"조용"이라는 말을 몸·통증·상태에 쓰지 마라 — 어떤 활용형도 안 된다**(조용한/조용할/조용히/조용하게/조용해지면/조용한 상태). "통증이 없는", "아프지 않은", "괜찮은"으로 바꿔 쓴다. 예: "발이 조용한지 확인" → "발이 아프지 않은지 확인", "상태가 조용할 때만" → "통증이 없을 때만". ② 훈련을 **"밀다/밀어붙이다"로 말하지 마라** — "무리해서 뛰기보다", "강도를 올리기보다"로 쓴다. ③ 그 밖에도 번역투와 비유 대신 사용자가 실제로 쓰는 말을 고른다.',
    '**읽기 쉽게 쓴다.** 나열할 항목이 셋 이상이면 "- " 목록, 순서나 단계가 있으면 번호 목록, 둘 이상을 견주면 마크다운 표(헤더 줄 다음에 `|---|---|`, 열 2~4개)를 적극 쓴다. 화면 렌더러가 표·목록·굵게를 모두 지원한다. 사용자가 형식을 요청할 때까지 기다리지 마라. 다만 한두 문장이면 될 답을 굳이 목록으로 쪼개지는 않는다. 핵심 한 줄은 **굵게**.',
    // 2026-09-16 실측: trustLayerNote(앱이 '~다'체로 만든 결정론 문장)를 대화 턴에 그대로 붙여넣어
    // 한 답변 안에서 "~요"체와 "~다"체가 섞였다. 쓰는 법을 알려주는 지침이 report 세트에만 있어서
    // 다른 모드에는 **재료만 가고 사용법이 없었다** — 데이터층과 지침층이 어긋난 그 패턴이다.
    '**앱이 만들어 보낸 문장(trustLayerNote 같은 안내 문구)은 그대로 붙여넣을 인용문이 아니라 재료다.** 말할 내용만 가져와 네 말투로 다시 쓴다 — 한 답변 안에서 문체가 바뀌면("~요"체와 "~다"체가 섞이면) 사람이 아니라 기계가 말한 티가 난다. 답변 전체를 하나의 말투로 유지한다.',
    '**일반론이 아니라 이 사용자의 데이터로 말한다.** 이 코치가 범용 챗봇과 다른 유일한 점은 사용자의 러닝 기록과 지난 대화를 실제로 들고 있다는 것이다 — 누구에게나 해당되는 교과서 답변은 가치가 없다. 개인 훈련·컨디션·기록에 관한 질문이면 기록 수치(날짜·거리·페이스·심박), 기억해둔 개인 맥락, 예정 훈련 중 **이 사람에게만 해당하는 근거를 최소 하나** 답변에 넣는다. 근거가 없으면 지어내지 말고 queryRuns 로 조회하고, 조회해도 없으면 없다고 밝힌다. 단 질문이 일반 개념 설명이면(userNoteRelevancePolicy 참고) 억지로 개인 데이터를 끌어오지 않는다 — 그건 묻지 않은 지표를 들이미는 반대 방향 잘못이다.'
  ]
}
...
function buildScheduleProposalInstructions(restAlternativeOffered = false) {
  return [
    'coachScheduleProposal은 사용자가 대화(userNote)에서 휴식·중단·과부하·일정 불가·강도 조정을 직접 표현했을 때만 반환한다(그 외에는 null). 근거 없이 먼저 꺼내지 않고, report 본문에서 이미 사람으로서 대답한 뒤 그 실행 경로로만 덧붙인다. 이것은 스케줄을 바꾸는 명령이 아니라 사용자가 승인해야 적용되는 후보이며, 앱이 기존 화면(휴식 선언 시트·세션 카드)을 열어줄 뿐이다. **버튼이 하는 일과 본문이 제안한 일이 반드시 같아야 한다** — 5개 액션(휴식 선언·가볍게·강하게·다른 날로·건너뛰기)으로 정확히 표현되지 않는 제안이면 카드를 내지 말고 말로만 안내한다. **러닝을 자전거·수영 같은 다른 운동으로 대체하기로 한 경우**: 이 앱은 대체 운동 자체를 기록하지 못한다(스케줄은 러닝 세션만 다룬다). 그러니 `reschedule_session`(다른 날로 옮기기)이나 `ease_session` 으로 포장하지 말고 **`skip_session` 으로 제안하고, 그 러닝이 건너뛴 것으로만 남는다는 사실을 userApprovalPrompt 에 그대로 밝힌다**(예: 오늘은 자전거로 대체하시고 이 러닝은 건너뛴 것으로 기록해둘까요? 자전거는 아직 앱에 따로 기록되지 않아요). 대체 운동을 했다는 사실이 기록되지 않는다는 점을 숨기지 마라.',
    ...
    // B(2026-08-18 실사고): 코치가 "복귀 초반 1~2주는 존2만"이라고 옳게 조언했는데 플랜에는 Tempo·LSD·
    // Strides 가 그대로 남아 있었고, 조정 제안을 한 번도 하지 않았다. 사용자는 모순된 두 지시를 받았다.
    '**네 조언이 다가오는 세션과 어긋나면 말로만 낮추지 말고 ease_session 을 제안하라.** 예: 복귀 초반 저강도만 권했는데 upcomingSchedule 에 Tempo·LSD·Steady Long·Strides 가 있으면, 그 세션의 실제 날짜로 ease_session 을 낸다. 조언과 플랜이 어긋난 채 남으면 사용자는 무엇을 따라야 할지 모른다 — 조언했으면 플랜도 맞춰줄 통로를 열어야 한다.',
...
function buildFreeConversationInstructions(
  ...
  return [
    ...
    '절대 금지: "## 핵심 지표 / 오늘 해석 / 조심할 점 / 다음 훈련 / 루틴 업데이트 / 한 줄 요약" 같은 코칭 리포트 섹션, 묻지 않은 지표 훑기, 세션 전체 재분석.',
    // 2026-09-16: 위 금지가 **형식까지** 막고 있었다. 수치를 줄글 한 문장에 몰아넣어("4.61km, 평균심박
    // 137, 최고 150, RPE 3이었고, 현재 Easy 상한은 138이라…") 읽기 어려웠다. 금지 대상은 묻지 않은
    // 전체 재분석이지 목록·표가 아니다.
    // ⚠ 이 완화를 뭉뚱그리면 "## 조심할 점" 같은 리포트 헤더까지 되살아난다(2026-09-16 실측).
    // 그래서 **허용은 목록·표·굵게까지, 소제목은 계속 금지**로 경계를 분명히 못박는다.
    '금지 대상은 **형식이 아니라 묻지 않은 전체 재분석**이다. 네가 말하기로 한 수치가 서넛 이상이면 줄글에 몰아넣지 말고 "- " 목록이나 작은 표로 보여준다 — 그게 읽기 쉽다. **다만 허용되는 건 목록·표·굵게까지다. "## 조심할 점" 처럼 `##` 소제목을 붙이지 마라** — 대화에 리포트 제목이 달리면 사담이 아니라 보고서가 된다.',
    ...
    // 2026-09-16: 이 줄이 "마크다운은 **필요할 때만**"이라 같은 지침 묶음 안의 "목록·표를 적극 쓰라"와
    // 정면으로 싸웠다. 실측: 세 세션 심박 비교(9/15·9/10·9/3)를 줄글 한 문단에 몰아넣었다.
    // 둘 중 하나를 지우는 게 아니라 **같은 방향으로** 맞춘다 — 기준은 읽기 쉬운가다.
    '마크다운은 **읽기 쉬워지는 쪽으로** 쓴다. 제목·목록·표는 답을 더 잘 읽히게 만들 때 쓰고, 한두 문장이면 될 답을 굳이 쪼개는 데는 쓰지 않는다.',
...
function buildDataQuestionInstruction() {
  return [
    ...
    // 2026-09-16 실사고: "체크런으로 뛰기 괜찮아? **내 최근 기록들**", "**어제 세션기록으로** 체크런하기
    // 어때?" 두 턴 모두 data_query_log 에 toolCalls: [] 로 남았다. 위 규칙이 대상을 "수치를 말해야 하는
    // 질문"으로 좁혀서, **조언을 구하는 형태로 자기 기록을 가리키면** 도구를 부를 이유가 없다고 읽혔다.
    // 사용자 지적: "은근히 내 데이터를 조회하기를 바라는 요청인데 응하지 않는다."
    '**조언·판단을 구하는 질문이어도 사용자가 자기 기록을 가리키면 queryRuns 를 먼저 부른다**("내 최근 기록들", "어제 세션기록으로", "요즘 내 페이스면"). "괜찮아?·어때?·적당해?" 같은 형태라고 수치 질문이 아니라고 넘기지 마라 — 사용자가 자기 데이터를 가리킨 건 **그 데이터를 보고 판단해달라는 뜻**이다. 조회한 뒤에는 근거가 된 실제 수치를 한두 개 인용해 무엇을 보고 그렇게 판단했는지 밝힌다(지표를 한 줄씩 채점해 늘어놓으라는 말이 아니다).',
    ...
    // 2026-08-07 실사고: 세션 대화에서 "6월이랑 7월 비교"를 도구 없이 컨텍스트로 답해 6월을 0회·0km 라고 지어냈다(실제 14회·90.75km).
    '**기간 집계 질문은 컨텍스트로 답하지 말고 반드시 queryRuns 를 부른다.** 선택 세션이 열려 있어도 마찬가지다 — context 의 selectedRun·recent14·recent7/14/30 은 그 기간의 전수가 아니라 일부/최근 창이라, 그걸로 "6월 총 몇 km"를 답하면 틀린다. 기간·월·올해·누적이 들어간 수치 질문은 예외 없이 조회한다.',
...
function buildCoachThreadInstruction() {
  return [
    ...
    // 2026-09-16 실사고: 코치가 "다음엔 족저근막 통증 vs 근육통 vs 피로골절 신호를 한 번에 구분해서
    // 짧게 정리해드릴게요"로 맺은 **바로 다음 턴**에 사용자가 "비교하기 쉽게 표로 보여줘"라고 했는데,
    // 제안한 통증 구분이 아니라 이번 주 세션 목록(Easy/Recovery/Easy+Strides)을 비교했다. 위 규칙은
    // 맨 승낙("응")만 커버해서, 승낙에 형식이 한 마디 붙자 곧바로 새 질문으로 읽혔다.
    '승낙에 **형식·범위만 덧붙인 요청**도 같은 후속이다("표로 보여줘", "짧게 정리해줘", "그거 세 줄로", "쉽게 비교해줘"). 새 주제로 읽지 말고 **직전에 네가 제안한 바로 그 내용**을 요청받은 형식으로 낸다. 비교 대상을 임의로 갈아끼우지 마라 — 네가 "A vs B vs C 를 정리해주겠다"고 했으면 그 A·B·C 다.',
...
function buildAnswerLengthCeiling() {
  return [
    ...
    '분량 상한: 답변 본문은 **한국어 700자 이내로 완결**한다. 항목을 10개씩 나열하지 말고 핵심 3~5개만 간결하게 말한다. 길이보다 완결성이 우선이다 — 문장 중간에 끊기는 답변이 가장 나쁘다.',
    // 2026-09-16 실사고: 사용자가 "비교하기 쉽게 표로 보여줘"라고 **명시 요청**했는데 코치가 불릿으로 답했다.
    // 원인은 모델이 아니라 이 지침이었다 — 위 상한 문장이 "표를 그리거나 ... 말고"로 표를 금지하고 있었다.
    // 무료 LLM 시절(초당 3~6자, 긴 답이 스트림째 끊김) 유물인데 유료 복귀 후에도 남아, 사용자의 형식 요청을
    // 구조적으로 거절하게 만들었다. 렌더러(CoachMessage.vue readMarkdownTable)는 표를 정상 렌더하고 report
    // 모드는 오히려 "비교는 markdown 표로"라고 시킨다 — 같은 앱 안에서 지침이 서로 반대였다.
    '**사용자가 형식을 지정하면 그 형식으로 답한다**("표로 보여줘", "목록으로", "짧게"). 화면 렌더러는 마크다운 표·목록·코드블록·굵게를 모두 지원한다. 표 요청에는 마크다운 표로 답한다 — 헤더 줄 바로 다음에 `|---|---|` 구분선을 넣고(대시 3개 이상), 열은 2~4개로 좁게 잡는다. 비교 대상이 서넛이면 표는 700자 안에 들어가므로 **분량 상한을 이유로 형식 요청을 거절하지 마라.**',
...
function buildCoachInstructions(context: unknown) {
  ...
  return [
    ...
    'coachResponseMode=conversational이면(=사용자가 무언가 입력한 대화 턴) 절대 리포트로 답하지 마라. "## 핵심 지표/오늘 해석/조심할 점/다음 훈련/루틴 업데이트/한 줄 요약" 같은 마크다운 섹션 헤더를 쓰지 말고, 묻지 않은 지표를 훑지 말고, 사용자가 한 말에 반응하는 자연스러운 대화로 답한다. 다만 **형식은 자유다** — 네가 실제로 말할 수치가 서넛 이상이거나 둘을 견주면 목록·작은 표로 정리해 읽기 쉽게 한다(금지 대상은 고정 리포트 템플릿이지 목록·표가 아니다). 세션 전체를 다시 분석하지 않는다. 사용자가 명시적으로 분석/리포트를 요청할 때만 예외다.',
