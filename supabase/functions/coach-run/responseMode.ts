...
function asksPeerComparison(text: string): boolean {
  // ① 이 어휘 자체가 "대회 분포와의 비교"를 뜻한다 — 다른 단서가 없어도 통과시킨다.
  if (/완주자|퍼센타일|백분위|등수|순위|또래|남들|상위\s*몇|몇\s*(%|퍼센트|프로)/.test(text)) return true

  // ② "비교"·"기준으로" 는 흔한 말이라 대상이 러닝/기록/사람집단일 때만 통과시킨다.
  //    `평균` 을 단독으로 받으면 "평균 수면시간이 러닝에 영향을 줘?" 같은 일반 개념 질문이 끌려온다.
  const comparesToOthers =
    /비교|기준으로|평균\s*기록|일반\s*(인|사람)|보통\s*(사람|러너)|다른\s*(사람|러너)/.test(text)
  if (!comparesToOthers) return false
  return /기록|페이스|완주|대회|마라톤|하프|풀코스|\d+\s*(k|km|킬로|키로)|러너|러닝|달리|뛰|예상|실력|수준|남자|여자|남성|여성|성별|전체|\d+\s*대|\d+\s*세/.test(
    text
  )
}
