 ...
 *
 * ⚠ `\d{1,2}\s*월` 뒤에 한글 금지 조건을 걸었더니 **"6월은 / 7월엔 / 6월에"를 통째로 놓쳤다**
 * (2026-08-07 실측: 세션 대화에서 "6월은 총 0회·0km"라고 지어낸 답변을 게이트가 못 잡았다).
 * 앞에 숫자가 붙은 "월"은 이미 요일과 구분되므로(“6월요일”이라는 말은 없다) 그 가드가 불필요했다.
 * "3개월"은 숫자와 월 사이에 '개'가 있어 이 패턴에 걸리지 않고, 별도 대안이 받는다.
 */
const UNGROUNDED_PERIOD =
  /지난\s*달|저번\s*달|전\s*달|이번\s*달|\d+\s*(달|개월)\s*전|\d+\s*개월\s*(간|동안)|\d{1,2}\s*월|작년|재작년|올해|\d{4}\s*년/
