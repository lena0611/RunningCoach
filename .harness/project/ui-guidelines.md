...
- Root tab pager panels stay mounted for swipe continuity. Any `position: fixed` or viewport-level sticky element inside a tab must be scoped to the active panel, or explicitly hidden when its `.tab-swipe-panel` has `aria-hidden='true'`, so Run Log headers or other tab-local chrome never leak into Dashboard, Trends, or Memory.
