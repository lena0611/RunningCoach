...
- Memory injury management must normalize body parts. Use the shared body selector: an anatomical render PNG placed as `<image>` inside the same `<svg>` as the region shapes (shared viewBox), with 앞/뒤/발바닥 views where each body-part shape is itself the touch target, plus a collapsed `이름으로 찾기` list fallback. Do not reintroduce rotatable degree-step stills, an image laid out separately from its hit-zones, a two-step "tap region then pick a candidate" flow, or free-text-only injury area entry; free text belongs in notes, triggers, and management details. Spec detail lives in `.harness/project/ui-system-contract.md`.
