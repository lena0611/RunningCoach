...
- `RunLog` 상세 화면은 평균 페이스, 평균 케이던스, 평균/최고 심박, RPE, 드리프트를 명시 라벨로 보여준다. 랩 데이터가 있으면 `스플릿` 컨텐츠로 승격하고, 목록/차트 전환을 제공한다. `routePoints`가 있으면 경로 섹션은 지도, 시작/종료 노드, 선택 구간 노드만 담당한다. `metricSamples`나 `routePoints`가 있으면 `스플릿 > 차트`에서 페이스/심박수/케이던스/고도 시간축 차트를 같은 드래그 선택 체계로 제공해 사용자가 러닝 중간 과정과 세션 유형 자동 해석 근거를 확인할 수 있어야 한다.
