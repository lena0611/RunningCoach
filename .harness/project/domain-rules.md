...
- MyResult처럼 공개 개별 상세 API를 이용해 컷을 산출하더라도, 작업 산출물과 제품 코드에는 참가자별 원본 응답, 이름, 배번, 개별 기록 row를 남기지 않고 집계 완료 수와 percentile cut만 반영한다.
- 비식별 집계 축에는 전체 외에 **성별 분포 컷(`genderDistribution`의 남/여 cut + sampleSize)**도 포함할 수 있다(동일한 재식별 불가 집계 원칙). 충분한 표본(최소 수백 명) 세그먼트만 노출한다. **나이대(연령)별 컷**은 출처(MyResult 공개 API)에 연령 데이터가 없어 현재 보류하며, 연령을 노출하는 출처(B.A.A. division, SCC AK)를 실제 수집할 때 추가한다. 수집은 재현 가능한 `tools/benchmark-harvest`로 하고 net time·성별만 메모리 집계 후 컷만 산출한다.
- 현재 목표 거리의 ready 대회가 1개뿐이면 `현주소` 단정이 아니라 `공식 대회 1개 참고`로 낮춰 표현한다. ready 대회가 2개 이상일 때만 `대회 기준 현주소`처럼 복수 기준 비교로 노출한다.
...
- 레벨은 **2축**이다. ① 거리 클래스(전직 축): 입문 → 5K → 10K → 하프 → 풀(→ 울트라). ② VDOT 등급(실력 축): 아이언/브론즈/실버/골드/플래티넘/다이아. 표기는 "10K 러너 · 실버". 거리만으로는 꼭대기에서 실력을 변별할 수 없어 등급축이 필수다(상대 레벨로 실력을 가늠하는 요구).
- 거리 클래스는 **최고 완주 거리**가 결정하며 **영구 해금**(강등 없음)이다. 어떤 연속 완주든 그 거리를 채우면 해금하는 포용적 기준이며, `self-race` 태그는 인증/등급 측정 근거로만 쓰고 클래스 해금의 필수 조건은 아니다.
- VDOT 등급은 **절대 밴드**라 거리와 무관하게 비교 가능하다(같은 등급=같은 실력). 등급은 **최근 측정 VDOT**로 계산돼 자연 감쇠하며, 측정 근거가 오래되면(기본 28일) 폼 점검(유지 퀘스트)을 권한다. **강등은 클래스가 아니라 등급/폼에만** 적용한다.
- 기존 `runnerLevel`(초/중/고)은 **VDOT 등급의 파생 뷰**다. 우선순위는 수동 override > 등급 파생 > (VDOT 없음) 기존 점수제 fallback이다. 등급→레벨 매핑으로 training KB의 `suitable_levels` 필터 호환을 유지하고, 기존 `deriveRunnerLevel`/`resolveRunnerLevel`은 fallback 경로로 보존한다(비파괴).
- 승급은 **2게이트**다. 게이트1(도전 자격)은 앱이 최장 주행·주간 볼륨·꾸준함·readiness로 자동 판정하는 **소프트 게이트**(경고만, 자격 미달도 도전 가능)이며 풀마라톤만 미달 시 강경고(부상 위험)다. 게이트2(완주 확정)는 GPS 연속 완주로 클래스를 영구 해금하며, 걷기가 섞여도 클래스는 인정하되 그때 페이스로 등급이 매겨진다.
- **무결성: 등급(실력)은 실측 성능(VDOT)으로만 상승**한다. XP·코인 같은 재화는 참여 보상이며 등급에 영향을 주지 않는다(랭킹 신뢰). 루틴 수행은 꾸준함을 통해 게이트1 자격에 기여할 뿐 등급을 직접 올리지 않는다.
- 온보딩 자기보고 배치는 **잠정(provisional)**이다. 자기보고 최장 거리가 실제 주행보다 높은 클래스를 만들면 잠정으로 표시하고 GPS 완주로 인증되면 확정한다. **마라톤 완주자를 입문부터 시작시키지 않는다.**
- 저장 계약: 거리 클래스·등급·자격은 **파생 계산이라 저장하지 않는다**. 저장은 `run_logs`로 복원 불가능한 상태에만 둔다 — 온보딩 자기보고 배치, 퀘스트 완료 로그, XP/코인 ledger(`pacelab_level_state` / `pacelab_quest_log` / `pacelab_reward_ledger`, 모두 RLS `auth.uid()`).
- 퀘스트 3종: 루틴(오늘/이번 주 훈련, 기존 추천 재프레임), 승급(다음 거리 도전 = 게이트1 → 게이트2), 유지(폼 재측정 타임트라이얼). 루틴은 `training_prescription_rules`의 거리(goal_distance) × 등급(suitable_levels) × phase로 생성한다.
- 코인 경제(#277): 단일 재화 = 코인. `pacelab_reward_ledger`(append-only)에 적립하고 잔액은 합계로 계산한다. 적립 단위 — 전직(클래스업) +200, 등급업 +50, 주간 루틴 완주(주간 러닝 횟수 ≥ `weeklyRunDaysTarget`) +30, 유지 점검(self-race 재측정 1건당, `quest_log` run_id 키) +20.
- 보상은 **전부 자동·idempotent**다. 수동 "완료" 버튼은 두지 않는다. 레벨업은 `pacelab_level_state.acknowledged_class`/`acknowledged_grade`와 현재 클래스/등급을 비교해 상승했을 때만 1회 적립하고 acknowledged를 갱신한다. 반복 보상(주간 루틴 등)은 `pacelab_quest_log(quest_type, quest_key)` 유일 키(주간은 그 주 월요일)로 중복 적립을 막는다.
- **첫 감지는 baseline**이다: acknowledged가 비어 있으면 현재 클래스/등급을 보상·축하 없이 acknowledge만 한다(기존 사용자 소급 축하 방지). 보상 동기화는 run/memory가 완전히 로드된 뒤에만 실행해 로드 경합으로 인한 가짜 레벨업 축하를 막는다.
- **무결성: 코인은 참여 보상이며 등급(실력, 실측 VDOT)에 영향을 주지 않는다.** 코인을 모아도 등급/클래스는 오르지 않는다(랭킹 신뢰). 승급/등급업 시에만 축하 모달을 1회 노출한다.
...
- `코치`는 독립 하단 탭이다(2026-07 리디자인 ①b에서 신설 — 이전 "독립 탭 금지" 규칙 폐기). 코치 탭은 코칭 엔진의 산출(세션 브리핑, 주간 캐러셀, 훈련 페이즈, 레벨, 퀘스트, 코치 모먼트)을 담당한다. 단, 세션별 AI 코칭 "대화"는 여전히 `RunLog` 상세에서 특정 세션 맥락을 잡고 진입하며, 같은 세션에서는 ChatGPT처럼 추가 대화를 이어간다(App 레벨 오버레이).
- Upload는 주요 하단 탭이 아니라 기록 추가 액션이다. 하단 네비는 홈/기록/메모처럼 반복 사용 화면 위주로 두고, 업로드/수동 추가는 `Run Log`나 관련 화면의 보조 액션으로 진입시킨다.
- 하단 메뉴의 정보 구조는 러너의 질문으로 구분한다(5탭: 요약·코치·기록·추세·기억). `요약`은 "오늘 무엇을 해야 하는가", `코치`는 "왜 이 훈련인가 / 계획은 무엇인가", `기록`은 "내가 무엇을 어떻게 뛰었는가", `추세`는 "나는 어떻게 변하고 있고 무엇을 조정해야 하는가", `기억`은 "앱과 코치가 나를 어떤 기준으로 알고 있는가"에 답한다. 그룹핑 원칙: 기억=코칭 엔진의 입력, 코치=산출.
...
- `기억`은 코치의 장기 맥락 관리 화면이다(리디자인 ①c에서 축소). L1은 "현재 코칭 기준 요약(활성 목표+제약)" + 관리 nav 4항목(목표 · 몸 상태 · 훈련 기준 · AI 기억)만 두고, 편집은 전부 drill-in 패널에서 항목별 저장으로 처리한다(전역 저장 버튼 금지). 러너 프로필·PB·업적은 기억 탭이 아니라 우상단 계정 드로어(AppHeader) 소관이다. 최근 기록 목록이나 세션 상세 차트를 반복 표시하지 않는다.
