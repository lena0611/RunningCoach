...
- 부상 체크인은 사용자가 Memory 화면을 직접 열 때만 의존하지 않는다. 앱 기동, 포커스 복귀, 롱런/Tempo/Strides 같은 품질 세션 다음 앱 사용 시점에 active 또는 monitoring 부상 항목이 있으면 통증 변화와 완치 후보 여부를 짧게 확인한다. 단, 매 기동마다 반복 질문하지 않도록 항목별 최근 체크 시각과 최근 강도 세션 이후 여부를 함께 보고, 사용자가 닫은 같은 사용자/같은 날/같은 품질 세션 조건은 앱 재시작 후에도 반복 노출하지 않는다.
