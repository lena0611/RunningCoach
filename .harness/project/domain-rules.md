...
- 코인 경제(#277): 단일 재화 = 코인. `pacelab_reward_ledger`(append-only)에 적립하고 잔액은 합계로 계산한다. 적립 단위 — 전직(클래스업) +200, 등급업 +50, 주간 루틴 완주(주간 러닝 횟수 ≥ `weeklyRunDaysTarget`) +30. (유지 점검 +20 예정)
