...
- 과거 세션 코칭의 "다음 훈련"과 "루틴 업데이트"는 `nextTrainingAdviceRelevant` 기준으로만 현재 처방으로 제시한다. 이 값은 세션이 코칭 요청 시점 기준 7일 이내(ATL 7일 시간상수 근거)이고 그 세션 이후 새 RunLog가 없을 때만 true다. false이면 다음 훈련은 그 시점 다음 스텝 회고 한 줄로 줄이고, 루틴 업데이트는 단일 과거 세션으로 현재 루틴을 바꾸거나 유지로 단정하지 않는다. 단일 세션 하나로 주간 루틴을 바꾸지 않는다는 기존 원칙과 일관된다.
