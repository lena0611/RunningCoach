...
- 특정 `RunLog`를 선택해서 AI 코칭을 다시 요청하면 새 리포트를 누적하지 않고 `(user_id, selected_run_id)` 기준으로 기존 `coach_reports`를 갱신한다. 이때 해당 리포트에서 파생된 `coach_memory_items`도 같은 `source_report_id` 기준으로 교체해 오래된 코칭 기억이 남지 않게 한다.
...
- AI Coach는 독립 하단 탭으로 두지 않는다. 코칭은 `RunLog` 상세에서 특정 세션 맥락을 잡고 진입해야 하며, 같은 세션 재요청은 기존 코칭을 갱신한다.
