...
- 도메인은 바로 새 하단 탭이나 독립 화면으로 승격하지 않는다. 먼저 기존 흐름의 입력 필드, 세션 태그, 코칭 컨텍스트, Memory 항목 중 어디에 붙는지 판단한다.
...
- 하단 메뉴의 정보 구조는 러너의 질문으로 구분한다. `요약`은 "오늘/이번 주 무엇을 해야 하는가", `기록`은 "내가 무엇을 어떻게 뛰었는가", `기억`은 "앱과 코치가 나를 어떤 기준으로 알고 있는가"에 답한다.
