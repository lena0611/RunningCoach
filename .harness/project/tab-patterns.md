...
| 예정 세션 | 타입색 숫자(채우지 않는다) |
| 완료 | 타입색 테두리(`--weekstrip-done-ring`) |
| 선언 휴식 | 점선 테두리(`--weekstrip-rest-ring`) |

**채움은 선택된 날 전용이다.** 다른 날까지 타입색으로 채우면 일곱 칸이 색 덩어리가 돼 '지금 보고 있는 날'이 묻힌다(2026-09-03 실사용 지적).
...
- **탭 라벨은 줄바꿈 금지**(`.st-item` `white-space: nowrap`) — 좁은 컨텍스트(카드 헤더 actions 등)에서 한글이 글자 단위로 꺾여 2줄 탭이 되는 것 방지(2026-07-10 '스플릿' 실기기 사례). `detail` 보조 설명만 줄바꿈 허용.
