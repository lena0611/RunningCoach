...
- **세션 간 최소 간격(minGap) — 하한 ~5h, 권장 7~9h.** 두 세션은 최소 5시간, 권장 7~9시간 벌린다. 이보다 짧으면 회복·글리코겐 재합성 창이 부족하다. 둘째가 강도 워크아웃이면 간격을 더 본다(둘째는 이지가 원칙 — 아래). 근거: Marathon Handbook("at least five hours", optimal 7~9h) https://marathonhandbook.com/running-twice-a-day/ . ⚠ 더 짧은 "4h" 류 하한은 권위 출처가 약한 실무 휴리스틱이라 단정하지 않는다(코칭 신뢰 원칙). 네이티브 앱은 minGap 미충족 시 둘째 세션 시작을 **하드 차단**한다(런타임 가드 — 웹 처방/표시와 iOS 시작 가드를 원자적으로 동시 변경, [[native-repo-git-management]]).
