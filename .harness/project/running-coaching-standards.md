...
두 번 뛰기(two-a-day)는 볼륨을 늘리는 **고급 도구**이지 초보의 기본기가 아니다. PaceLAB은 더블을 자격을 갖춘 러너의 따라잡기(make-up)·고볼륨 수단으로만 좁게 처방하고, 회복 간격으로 부상·과훈련을 막는다. 수치는 절대규칙이 아니라 가드레일이다(§기준선과 일관). (결정: `decision-log.md` 2026-06-21, /grill-me 제품 합의 — 진입점=코치 자동제안+수동추가 둘 다, 범위=같은 날 AM/PM 2세션까지(N≥3 보류), 네이티브 minGap=하드 차단)
...
- **세션 간 최소 간격(minGap) — 하한 ~5h, 권장 7~9h.** 두 세션은 최소 5시간, 권장 7~9시간 벌린다. 이보다 짧으면 회복·글리코겐 재합성 창이 부족하다. 둘째가 강도 워크아웃이면 간격을 더 본다(둘째는 이지가 원칙 — 아래). 근거: Marathon Handbook("at least five hours", optimal 7~9h) https://marathonhandbook.com/running-twice-a-day/ . ⚠ 더 짧은 "4h" 류 하한은 권위 출처가 약한 실무 휴리스틱이라 단정하지 않는다(코칭 신뢰 원칙). minGap 강제는 **두 단계**다: v1 은 웹이 오전 런 실제 종료시각 기준으로 권장 오후 시작 시각·빠듯/양호를 **동적 안내**하고(인앱 '시작' 이벤트가 없어 — 훈련 런은 워치→import — 인터랙티브 차단은 두지 않는다), 실제 시작을 관측해 둘째 세션 시작을 **하드 차단**하는 런타임 가드는 네이티브 후속으로 둔다(웹 처방/표시 ↔ iOS 시작 가드 원자적 동시 변경, [[native-repo-git-management]], #462).
