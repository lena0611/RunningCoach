...
- **주말 트리아지(주 마감 임박 — 실시간 코칭의 절반) — 살리고 놓아준다.** 주 마감이 임박했는데(남은 1~2일) open/포기 백로그가 남은 날 용량으로 다 못 따라잡힐 때, 코치는 닦달·크래밍을 하지 않는다. **그 주의 키 세션 하나를 살릴 날로 옮기길 제안하고 나머지는 죄책감 없이 놓아준다**(키 세션 우선 — 위 재배치 규칙; 회복은 훈련의 일부 — §기준선). 이 한 번의 트리아지는 "루틴 변경"이 아니다 — §루틴 변경 기준의 "2주 이상 핵심 세션 누락 반복"에 해당하는 건 **여러 주에 걸친 반복 패턴**이지 한 주의 혼잡이 아니다.
