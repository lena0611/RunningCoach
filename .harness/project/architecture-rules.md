...
- 부상관리는 `injuryItems`와 `activeInjuryItemId`를 단일 기준으로 한다. 자유 텍스트 레거시 필드(`knownIssues`·`runningStyle`·`heatStrategy`)는 후속 구조화 필드(부상 항목·`runnerIdentity`·날씨 코칭)로 대체돼 2026-09-07 제거했다. 저장된 옛 JSON 의 잔재는 컨텍스트 진입 지점(coach-run `sanitizeMemoryHeartRateCeilings`)에서 떼어낸다.
...
- AI 코칭 요청 -> Supabase Edge Function -> DB에서 `TrainingMemory`, `RunLog`, `coach_memory_items` 조회 -> OpenAI 호출 -> `coach_reports`, 새 `coach_memory_items`, 필요한 경우 `training_memory.memory`(전략 노트·aiNotes 등) 저장
...
- **관측 로깅을 모델의 자발적 호출에 걸지 않는다.** `coach_data_gaps`는 모델이 `queryRuns`/`reportDataGap`을 먼저 불러야만 기록되는 구조라, 모델이 도구 없이 깔끔하게 거절한 케이스가 통째로 새어나갔다(2026-08-05~24 대화 75건 동안 신규 0건 — **코치가 정직하게 거절할수록 기록이 사라진다**). 실패를 기록하려 하지 말고 **일어난 일을 턴마다 남기고**(`coach_reports.data_query_log`) 실패는 사후 조회로 도출한다. 저장할 것은 오프라인에서 되살릴 수 없는 것뿐이다 — 발화·시각은 같은 행에 있고 응답 모드·발화 분류는 발화로 재계산된다.
