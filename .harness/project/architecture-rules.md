...
- iOS 기상 확장: 현재 위치 -> 네이티브 iOS WeatherKit 조회 -> 웹 앱에 `WeatherSnapshot` 전달 -> 홈의 다음 세션 준비 카드에서 체감온도/강수확률/강수량/강수시간 표시
...
## WeatherKit 브리지 계약
- WeatherKit 데이터 구조와 웹 전달 구조는 `.harness/project/weatherkit-data-contract.md`를 기준으로 한다.
- 네이티브는 Apple WeatherKit과 CoreLocation 권한/조회만 담당하고, 웹 앱은 `WeatherSnapshot` plain JSON만 받는다.
- 웹 앱의 WeatherKit 브리지 등록과 자동 갱신 트리거는 앱 루트 전역 스토어에서 담당한다.
- WeatherKit이 실패해도 RunLog 저장, HealthKit 동기화, AI 코칭은 계속 동작해야 한다.
...
- HealthKit/WeatherKit 연동은 서버리스나 웹 브라우저 코드가 아니라 iOS 네이티브 타깃에서 구현한다. 웹 앱은 HealthKit/WeatherKit 권한 요청이나 직접 조회를 담당하지 않는다.
