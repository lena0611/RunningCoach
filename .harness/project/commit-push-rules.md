...
- 구현/버그/운영 요청을 사용자가 맡긴 경우, 명시적인 보류 지시가 없으면 commit, push, PR 생성, main 머지, 배포 확인, Issue 완료처리까지 진행할 수 있습니다.
...
## PR과 배포
- MVP Target의 정식 Issue에서는 `진행해`와 구현/버그/운영 작업 위임을 완료 흐름 승인으로 봅니다. 별도 중단 지시가 없으면 PR merge, 배포 확인, Issue Done까지 이어갑니다.
...
- MVP 기본 흐름에서는 검증과 배포 확인이 끝나고 후속 확인이 없으면 Issue에 완료 요약을 남기고 Status를 `Done`으로 바꾼 뒤 Issue를 닫습니다.
