...
1. **UI/화면 변경 = 사람 시각 검증(스크린샷 루프).** 자동 E2E(Playwright)는 **쓰지 않는다** — 이 환경에선 vite/esbuild 빌드 + chromium이 보안 에이전트(AhnLab/Jiran) 실시간 스캔과 Spotlight 인덱싱을 폭주시켜 맥을 느리게 한다. 대신:
   - **사용자에게 "확인해 달라"고 요청하기 전에, 반드시 Pages 배포 런 success를 확인한다.** `gh run list --workflow=pages.yml`로 HEAD 커밋의 런이 `completed success`인지 본 뒤 보고한다. **머지 ≠ 배포 완료**(Pages 빌드 1~2분). 배포 확인 전 "확인해 보세요"라고 말하지 않는다.
   - 배포 확인 후 **사용자가 실기기/브라우저로 눈으로 확인**하고, 이상하면 **스크린샷을 제공**한다.
