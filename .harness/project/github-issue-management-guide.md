...
작업 완료 후 `main`에 머지되고 더 이상 필요 없으면 worktree와 머지된 브랜치를 함께 정리합니다:

```bash
cd /Users/smart-tn-083/practice/run-ai
git -C ../run-ai.worktrees/issue-4-github-issue-guard status -sb
...
worktree 정리:
- 제거: /Users/smart-tn-083/practice/run-ai.worktrees/issue-00-example
```

검증을 못 했다면 `검증`에 실행하지 못한 이유와 남은 확인 방법을 씁니다. 후속이 있으면 새 Issue를 만들고 링크합니다.
worktree를 제거하지 않았다면 `worktree 정리`에 `보류: Issue Open/Deployed`, `보류: 미커밋 변경`, `보류: 후속 확인 필요`처럼 사유를 남깁니다.
...
Issue가 `Closed`이고 Project Status가 `Done`이며 해당 Issue worktree가 clean이면 완료 후 정리 대상입니다. 기준 작업트리에서 `git worktree remove <경로>`와 `git worktree prune`을 실행하고 결과를 댓글에 남깁니다. `Open` 또는 `Deployed` 상태인 Issue worktree는 사용자 최종 완료 확인 전까지 삭제하지 않습니다.
