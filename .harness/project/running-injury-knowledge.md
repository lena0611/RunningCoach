...
- **통증 정지 규칙(진행 금지).** 러닝 중 ①날카로운 통증 ②지속할수록 악화되는 통증 ③보행을 바꿀 만큼 심한 통증 중 하나라도 발생하면 단계 진행 금지·중단.
- **정량 판정 = Pain-Monitoring Model (2026-09-16 확정, 위 공백 해소).** 계보는 Thomeé(슬개대퇴) → **Silbernagel 2007 아킬레스 건병증 RCT** → 슬개건병증 확장. 규칙은 두 줄이다: ①**활동 중·직후 통증 ≤5/10**(NPRS) ②**다음 날 아침에 활동 전 수준(그 사람의 기준선)으로 복귀**. 이 모델로 활동을 *계속한* 군이 통증 회피군 대비 결과에 불리하지 않았다 — **"통증 0 이어야 뛴다"는 근거가 없다.** ⚠ 판정은 절대 0 이 아니라 **기준선 복귀**다: 아침 기준선이 3 인 사람에게 "0~2 면 관찰" 같은 고정 밴드를 들이대면 영원히 빨간불이 되고 부하가 적절했는지를 판정할 수 없다. 출처·전문은 `research/injury-pain-monitoring-and-prognosis.md`.
  - ⚠ **확정 못 한 것(규칙 금지)**: "주 대비 통증 증가 금지(week-to-week)"는 2차 요약에만 있고 1차 모델 기술에는 없다. 족저근막 전용 수치(예 4/10)도 1차 출처 없음. 모델의 검증 대상은 **건병증**이라, 족저근막증엔 "일반 부하 관리 원칙"으로 적용하고 질환 특이 처방으로 단정하지 않는다.
...
## 3-C. 부상 예후(natural history) — "언제까지?"에 답한다 (2026-09-16)

코치가 "조용해지면"만 반복하면 사용자는 **언제까지 이러고 있어야 하는지** 알 수 없다. 예후는 겁주기가
아니라 **기다릴 수 있게 하는 정보**다. 전문 근거는 `research/injury-pain-monitoring-and-prognosis.md`.

- **족저근막증 = 개월 단위 질환.** RCT 2차분석 n=200(PMC12266988): 활동 중 통증 6.31→**2.81**(12개월,
  d=1.56), FFI 45.36→21.84. **가장 큰 호전은 첫 3개월**, 이후 6·12개월에도 완만히 계속. 즉
  **12개월 시점에도 평균 2.81/10 이 남는다.** StatPearls: 75~80% 가 12개월 내 호전, 10~15% 는 2년 이상 만성.
- **아침 첫 체중부하 통증은 마지막까지 남는 축**이다(JOSPT 2023 CPG — 진단 지문이자 잔존 축). 낮에
  괜찮아졌다고 정상 복귀하면 재악화하기 쉽다.
- **양측성은 다른 하위군**(표본 38%): 6→12개월 구간에서 개선이 없었다. 양쪽이면 더 보수적으로 보고
  전문가 평가를 앞당길 근거가 된다.
- **코칭 적용**: "몇 주면 낫는다"고 말하지 않는다. "보통 개월 단위이고 첫 3개월에 가장 많이 좋아진다,
  아침 통증이 마지막까지 남는 게 흔하다"로 전한다 — 단정 금지, 개인차 명시, 의료 경계(§4) 우선.
- ⚠ **예후를 근거로 방치하지 않는다.** 정상 경과 안내와 §4 redFlag·escalation 은 별개다.

## 4. redFlag 안전 게이트 (처방보다 우선, 켜지면 처방 멈추고 의뢰 카피)
전역: 활동·시간 갈수록 심해지고 야간/휴식/보행통→피로골절 / 점통+hop재현 / 체중부하 곤란·부종·발적·열감·발열 / 저림·방사통·근력저하·발처짐 / 양측·전신·비기계적 야간통 / 6주(연부조직)~3개월(난치) 무호전. ⚠ **어느 쪽인지는 질환 경과로 정한다**(§3-C) — 족저근막증·건병증처럼 개월 단위가 정상 경과인 질환에 6주를 들이대면 정상 경과를 적신호로 부른다(과대 의뢰). 이 구분은 `loadTolerantTendinopathy` 로 KB 가설에 인코딩한다(같은 플래그가 §3-B 통증 허용 판정의 적용 범위도 정한다).
