...
## 언제 실행하나 (강제화 3층)
1. **작업 전(기본·의무, 예외 없음)**: 코칭 동작/지식을 바꾸는 작업은 착수 전에 코치 SSOT(`running-coaching-standards.md`, 부상 시 `running-injury-knowledge.md`)와 관련 메모리를 **반드시 먼저 읽는다**. 사용자 요청이 전문 코치 의도와 배치되면 **코드 짜기 전에** 사용자와 그릴(사전 조율) → 낭비 개발 방지. visible trace에 "코치 SSOT 확인됨"을 남긴다. (사용자가 `#전문코치리뷰`로 사전 점검을 요청할 수도 있다.)
