...
- GitHub Project `PaceLAB Development`는 전체 상태, 우선순위, workstream 라우팅, 완료 책임 창을 조감하는 보드입니다.
...
GitHub `Assignees`는 사람 계정 배정용입니다. PaceLAB의 업무 유형 라우팅은 `Assignees`가 아니라 Project의 `Workstream` 필드로 관리하고, 최종 완료 판단은 `Completion Owner` 필드로 관리합니다.
...
- `reset-needed`: 컨텍스트 혼선 위험이 커서 새 요청 창이나 분리된 Issue worktree로 넘겨야 합니다.
...
| `Ready` | 목표, 완료 조건, workstream 라우팅, 검증 후보가 정리되어 착수 가능합니다. |
...
사용자 프롬프트에 Issue URL 또는 Issue 번호가 있으면, 에이전트는 먼저 해당 Issue의 본문, labels, Project fields를 조회한 뒤 `Workstream`, `Completion Owner`, `Target`, `Verification`을 기준으로 읽을거리와 작업 범위를 정합니다. Issue를 조회하지 않은 상태에서 workstream이나 구현 범위를 추측하지 않습니다.

사용자 프롬프트가 새 업무 요청이면, 에이전트는 요청을 그대로 옮기지 않고 한글 우선 제목과 위 등록 기준을 채운 Issue 초안으로 구체화합니다. 같은 목표의 기존 Issue가 있는지 검색한 뒤, 없을 때만 새 Issue를 만들고 Project fields와 모바일 label을 맞춥니다. 제목은 가능하면 한글로 쓰되 `HealthKit`, `Supabase`, API명, 라이브러리명, 에러 메시지처럼 원문 식별이 중요한 고유명은 영어를 유지합니다.
