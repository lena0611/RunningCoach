...
- HealthKit 원본 샘플 전체를 장기 저장하지 않는다.
...
- `HKWorkoutRoute`
  ...
  - 앱 기본 MVP에서는 route 원본 좌표를 저장하지 않는다. 필요하면 거리/고도/분할 계산 후보 생성에만 사용한다.
...
- `laps`: HealthKit에서 lap/split 구조를 안정적으로 못 받으면 빈 배열을 허용한다.
- `source`: HealthKit 후보 저장 시 `healthkit`을 사용한다.

## 제한과 보완
- HealthKit은 Workoutdoors의 FIT 전체 필드를 1:1로 보장하지 않는다.
- Lap/split, Workoutdoors 고유 메타데이터, 일부 러닝 다이내믹스는 누락될 수 있다.
