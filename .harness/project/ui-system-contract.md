...
- 입력 필드 선택 기준:
  ...
  - 부상 부위처럼 해석 차이를 줄여야 하는 도메인 값은 자유 텍스트가 아니라 정규화 선택 UI를 사용한다. 부상관리는 `InjuryBodySelector`로 WebGL/Three.js 없이 동작하는 경량 SVG 스틸컷 around-view와 상체/하체/발 목록을 함께 제공하고, 복수 선택과 부위별 `ScaleSlider` 통증 레벨을 기본 패턴으로 둔다. 상체/허리와 하체는 45도 단위 9컷(0~360도)을 제공하고, 발/발목은 전면/후면/내측/외측/발등/발바닥처럼 러닝 부상 판단에 필요한 관점별 스틸컷을 제공한다. 기본 상태에서 부위를 미리 파란 덩어리로 채우지 말고 불투명 인체 표면 + 경계선만 보여주며, 선택/호버 시에만 파란 선택 효과를 준다. 향후 생성 이미지나 고품질 스틸컷 자산을 도입하더라도 컴포넌트 API와 선택 데이터 구조는 유지한다.
