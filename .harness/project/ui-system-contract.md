...
- typography: `--text-*`, `--font-weight-*`
  ...
  - **폰트 사이즈 토큰 용도(6+1단계, 아래 표대로 쓴다 — 화면마다 임의 판단 후 하드코딩 금지):**
    ...
    | `--text-micro-size` | 11 | 순수 장식 마이크로 텍스트(카운트 배지·eyebrow 태그) 전용 |

  - **최소 크기 하한(강제):** 사용자가 읽고 판단하는 텍스트(설명·근거·helper·지표 레이블)는 **최소 `--text-caption-size`(13px)**, 코칭 근거류는 **`--text-info-size`(15px)**. 10px·10.5px·12px 등 하드코딩 소형 폰트를 읽는 텍스트에 쓰지 않는다. 11px 이하는 `--text-micro-size`(순수 장식)만 허용.
