...
- typography: `--text-*`, `--font-weight-*`
  ...
  - **폰트 사이즈 토큰 용도(6+1단계, 아래 표대로 쓴다 — 화면마다 임의 판단 후 하드코딩 금지):**
    ...
    | 토큰 | 크기 | 용도 |
    |---|---|---|
    | `--text-display-size` | clamp 30~42 | 화면 최상위 히어로 숫자/타이틀 1개 |
    | `--text-metric-size` | clamp 28~38 | `StatCard`/`MetricGrid` 지표 숫자 전용 |
    | `--text-title-size` | 22 | 페이지/스택 헤더, `SectionHeader` |
    | `--text-body-size` | 16 | 본문 문단, 리스트 1차 텍스트 |
    | `--text-info-size` | 16 | 판단용 문장(코칭 근거·처방·복귀 기준·날씨 조언) — body와 동일 크기, 별도 시맨틱 |
    | `--text-caption-size` | 14 | 의미 있는 작은 정보(지표 레이블·부가 설명·타임스탬프 중 판단에 쓰이는 것) |
  ...
  - font-size는 위 토큰(`var(--text-*-size)`)을 참조한다. 신규/수정 CSS에서 13~15px대 하드코딩을 발견하면 해당 토큰으로 치환한다(13→caption 14, 15→info 16).
  - **폰트 하한/스케일 점검은 선언 형태 전부를 본다:** `font-size: NNpx`뿐 아니라 **`font:` shorthand**(`font: 700 10px/1 ...`), **rem/em**(0.875rem 미만=<14px, 0.75rem=12px), 브라우저 기본 `<small>`(상속으로 <14 될 수 있음)까지 포함한다. `font-size: px`만 grep하면 shorthand·rem·상속 케이스를 놓친다(#609 후속에서 트로피/레이스/용어집이 이렇게 누락됨). 확실히 하려면 정적 grep + **라이브 computed 스캔**을 함께 쓴다. **예외:** SVG 내부 `text{font-size}`는 viewBox 스케일되는 userspace 단위라 CSS px 하한(12px) 대상 아님.
...
- 탭/전환: `SegmentTabs`(콘텐츠 내 모든 탭 — variant 5종 underline/chips/segmented/pill/group × tone 3종 ok/accent/warning), `WeekStrip`(요약 홈 날짜 스트립), `BottomNav`(하단 글로벌 5탭). 스펙·토큰·사용 규칙·의도적 비이전 목록은 `.harness/project/tab-patterns.md`. 화면별 인라인 탭 재작성 금지.
...
- 입력 필드 선택 기준:
  ...
  - 부상 부위처럼 해석 차이를 줄여야 하는 도메인 값은 자유 텍스트가 아니라 정규화 선택 UI를 사용한다. 부상관리는 `InjuryBodySelector`로 WebGL/Three.js 없이 동작하는 **인체 렌더 PNG + SVG 부위 도형** 바디맵과 부위 목록(`이름으로 찾기`)을 함께 제공하고, 복수 선택과 부위별 `ScaleSlider` 통증 레벨을 기본 패턴으로 둔다.
    - **PNG는 반드시 `<svg>` 안 `<image>`로 넣어 도형과 viewBox를 공유한다.** `<img>`나 CSS background로 깔면 이미지는 `object-fit` 레터박싱을, 도형은 컨테이너를 따라 서로 밀린다 — 이전 구현(PNG 스틸컷 + 별도 사각 hit-zone)이 정확히 이 버그였고, 종횡비가 다른 상체 모델에서 특히 크게 어긋났다. viewBox를 공유하면 이 계열 버그가 원천적으로 불가능하다.
    - **부위 도형 자체가 터치 영역이다.** 기본은 투명(`fill: transparent` + `pointer-events: all`)이고 선택 시 반투명 초록으로 덮어 아래 근육 음영이 비쳐 보이게 한다. 그림과 hit-zone을 다른 좌표계에 두지 않는다.
    - **도형 좌표는 PNG 알파 마스크에서 추출한다.** 눈으로 맞추지 않는다 — 행마다 실루엣 경계를 읽어 폴리곤으로 만든다. 좌우도 미러가 아니라 각각 실측한다(렌더가 완전 대칭이 아니다). **그림을 교체하면 좌표를 다시 추출한다.**
    - 뷰는 **앞 / 뒤 / 발바닥 3개**다. 각도(0~360도) 회전 컷·회전 화살표·드래그 회전은 쓰지 않는다. 러너가 "내 아픈 곳"을 찾는 화면에 각도 숫자는 사용자 언어가 아니고, 9컷 중 0°와 360°는 같은 화면이었다.
    - 부위를 누르면 **바로 선택/해제**된다. 영역 터치 후 상세 후보를 다시 고르게 하는 2단 흐름을 두지 않는다.
    - 좌우는 **뷰마다 다르다**: 앞 뷰는 마주 본 상태라 화면 왼쪽이 사용자의 오른쪽이고, 뒤·발바닥 뷰는 화면 왼쪽이 사용자의 왼쪽이다. 혼동을 막기 위해 도형 위에 `왼쪽`/`오른쪽` 라벨을 항상 함께 렌더한다.
    - **viewBox는 뷰마다 다르다**(전신 `200×480`, 발바닥 `200×300` — 담을 비율이 다르다). SVG `<text>` 크기는 viewBox 배율을 타므로 라벨 `font-size`는 CSS가 아니라 뷰별 속성으로 준다. CSS에 박으면 속성을 덮어 발바닥 뷰 라벨만 커진다.
    - 도형 데이터는 `src/shared/ui/injuryBodyMap.ts` 단일 출처다. 부위를 추가하면 `injuryBodyMap.test.ts`가 카탈로그↔바디맵 전수 대응(미매핑·미존재 id·뷰 내 중복·좌우 짝)을 검사한다. 그림·좌표를 다시 만들어도 컴포넌트 API와 선택 데이터 구조는 유지한다.
    - **에셋 조달**: 이미지 생성 모델은 알파 채널을 못 만든다. "배경 투명"을 요구하면 흰색이나 얼룩 배경을 그려온다 — 크로마키 단색(`#FF00FF`)을 요구하고 키잉은 코드로 하거나, 알파가 실제로 들어있는지 **파일을 받아 canvas 픽셀로 확인**한다. 대화에 붙인 이미지로는 알파를 판별할 수 없다.
