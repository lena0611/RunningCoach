...
- 입력 필드 선택 기준:
  ...
  - 부상 부위처럼 해석 차이를 줄여야 하는 도메인 값은 자유 텍스트가 아니라 정규화 선택 UI를 사용한다. 부상관리는 `InjuryBodySelector`로 Three.js 기반 3D around-view 인체 모델과 상체/하체/발 목록을 함께 제공하고, 복수 선택과 부위별 `ScaleSlider` 통증 레벨을 기본 패턴으로 둔다. 3D 모델은 드래그 회전, 터치/raycast 선택, 선택 부위 하이라이트를 지원해야 하며 2D 도식으로 되돌리지 않는다.
