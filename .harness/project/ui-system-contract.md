...
- typography: `--text-*`, `--font-weight-*`
  ...
  - **폰트 사이즈 토큰 용도(6+1단계, 아래 표대로 쓴다 — 화면마다 임의 판단 후 하드코딩 금지):**
    ...
    | `--text-info-size` | 15 | 판단용 문장(코칭 근거·처방·복귀 기준·날씨 조언) |
    | `--text-caption-size` | 13 | 순수 메타데이터·타임스탬프·부가 라벨(판단에 안 쓰이는 것만) |
    | `--text-micro-size` | 12 | 앱 폰트 **하한(12px)**. 장식/메타·단순 정보성 마이크로 텍스트(카운트 배지·eyebrow·요일 라벨 등) |

  - **앱 전체 폰트 하한 = 12px(예외 없음).** 12px 미만(9/10/10.5/11px 등)은 어디에도 쓰지 않는다 — 가장 작은 장식/메타/단순 정보성 텍스트도 `--text-micro-size`(12px)가 바닥이다.
  - **최소 크기 하한(강제):** 사용자가 읽고 **판단**하는 텍스트(설명·근거·helper·지표 레이블)는 **최소 `--text-caption-size`(13px)**, 코칭 근거류는 **`--text-info-size`(15px)**. 단순 정보성/장식 라벨은 `--text-micro-size`(12px)까지 내릴 수 있다.
  ...
  - font-size는 위 토큰(`var(--text-*-size)`)을 참조한다. 신규/수정 CSS에서 13~15px대 하드코딩을 발견하면 해당 토큰으로 치환한다.
