...
### 남은 것
- **#806 기억 탭이 실제 기억을 안 보여준다** — "장기 메모 0개"인데 실제 121건. 화면은 `aiNotes` 만 세고
  `coach_memory_items` 는 **웹에서 조회조차 안 한다.** 이 화면이 #796 의 전제를 잘못 잡게 만들었다.
  목록만 볼지/지우기까지 넣을지가 설계 갈림길(UI 추가라 모양 합의 후).
