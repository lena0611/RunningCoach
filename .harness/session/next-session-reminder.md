...
## ⭐ 현재 위치 (2026-06-23) — iOS '새 러닝 감지' 런치 오탐 수정·기기검증·머지(PR#488) / 휴식·복귀(#473) Phase 1·2 코어 완료
- **이번 세션 완료: iOS '새 러닝 감지' 배너 런치 오탐(PR#488, 머지·기기검증 통과).** 새 빌드 후 첫 기동 시 새 러닝 0개여도 배너가 뜨던 버그 = HKObserverQuery 등록 직후 초기 콜백을 새 러닝으로 오인. 수정: `handleBackgroundHealthKitChange`를 `== .background` 일 때만 알림(`!= .active`면 런치 `.inactive`가 통과해 오탐) + `willPresent` id prefix 오타(`pacelab-healthkit-detected-`→`pacelab-healthkit-detected`). 검증=앱 삭제·클린 재설치→기동 시 배너 안 뜸. 상세 [[healthkit-detected-notify-gate]].
  - **남은 iOS 후속**: (1) **원래 #473 목표 = 워치 실주행→집 동기화 시 '제때 1번' 알림 오는지**(iOS 백그라운드 깨움 의존, **워치 실주행 테스트 필요**), (2) (선택) `.background` 팬텀 콜백까지 막는 "진짜 새 워크아웃 endDate 게이트". 순서=(1) 확인 후 (2). iOS '새 러닝 감지' 스택=연발 디바운스(PR#486)+런치 오탐(PR#488).
...
## 다음 1순위
1. **iOS '새 러닝 감지' 실주행 확인** — 가짜 배너는 제거됨(PR#488, 기기검증·머지). 다음 = **워치 차고 실제 1회 뛰어 집 동기화 시 '제때 1번' 알림이 오나** 확인(iOS 백그라운드 깨움 의존, 워치 실주행 필요). 오면 마무리, 미수신/잔존 오탐이면 "진짜 새 워크아웃 endDate 게이트" 착수. [[healthkit-detected-notify-gate]].
2. **#473 마무리 판단 + 후속 택1** — Phase 1·2 코어 완료·검증(인증 E2E)·배포 끝, 라이브 스모크는 E2E로 커버됨. 남은 후속: (a) **부상 복귀 walk-run 점진 처방**(현재 reason-blind 거리램프=공백, injury KB §3-B), (b) **coach-run LLM 휴식 인지**(채팅 닦달 구멍), (c) #473 이슈를 Phase 1·2 완료로 정리하고 (a)(b)는 별도 후속 이슈로 분리. 상세 [[rest-and-return-coaching]]·이슈 #473 코멘트.
3. **실기기 시각 스팟체크** — #462 강한 확인 오버레이(셀프레이스로 같은 날 둘째 시작 시) + #455 더블 카드 + 동적 gap 바(자연 발생 시, 위험 낮음).
4. **#454 나머지 플로우 실렌더 스팟체크** — 주 페이징·다른날로/스왑·포기 잔존·주말 트리아지. 통과면 에픽 #362 마무리.
5. **#359**(롱런 네거티브 스플릿) 토 LSD 스모크 · **#307** 인터뷰 스모크 · **#374** 주기화·개러지 실기기.
6. **grill 설계 백로그 정리** — 게이트 이슈(#260·#397·#411/#398/#408/#279/#375)·메모리-only 비전 → `needs:design-grill` 라벨/메모리 통합.
7. (이슈 미등록) 네이티브 fast-segment 임계 5:45→5:50 튜닝(수동 Xcode).
