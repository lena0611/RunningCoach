...
## ⭐ 현재 위치 (2026-09-08) — "두 번째 진실" 색출 + 기억 층 정리 + 보안 회차

한 줄 요약: **같은 사실을 두 곳에 두면 반드시 어긋난다** — 오늘 그 병을 다섯 군데서 잡았다.
사용자 질문 하나("코치가 내린 이지스트라이드가 앱과 다르다")에서 시작해 뿌리까지 갔다.
PR 10건(#794·#797·#798·#799·#800·#802·#803·#804·#805 + 세션기록). **전부 머지·배포·라이브 검증.**

### 오늘 출하 (전부 머지·배포·라이브 검증)
- **#767 후속 (PR#794) 옛 처방 템플릿·레거시 자유 텍스트 제거.** 기억 탭 "처방 템플릿"의
  Easy + Strides 가 `워밍업 10분 → 20초 가속/1분40초 회복 ×8 → 쿨다운 15분` — **이지 본런이 없는
  인터벌**이고 SSOT §Easy + Strides(본런 끝·15~20초·60~90초 걷기·반복수 산출)와 정면 충돌. 그런데
  프롬프트는 "다음 훈련 제안 시 이 템플릿 구조를 우선 보라"고 시키고 있었다(다른 줄은 반대 지시).
  실측: 사용자 템플릿 8종이 온보딩 시드에서 **한 글자도 안 바뀌어** 있었다 — "AI가 조정하는 기준"
  전제가 애초에 성립 안 했다. 함께 `knownIssues`·`runningStyle`·`heatStrategy` 제거,
  `aiNotes`(코칭 메모)는 **읽기 전용**으로. ⚠ **화면·스키마에서 없애도 프롬프트로는 계속 샌다** —
  `context.trainingMemory` 는 저장 블롭을 통째로 실으므로 진입 지점(`sanitizeMemoryHeartRateCeilings`)
  에서 키를 떼어내야 한다(테스트로 잠금).
- **#795 (PR#797) 세션 실행 지침을 코치에게 결정론 주입.** 위 제거 후에도 코치가 옛 숫자를 4턴 연속
  반복하고 **"앱 기준으로는"이라며 틀린 출처까지** 붙였다. 원인은 데이터 부재 — `upcomingSchedule` 이
  날짜·유형·거리만 실어 **"어떻게 뛰나"의 정본이 코치에게 없었다.** `buildSessionExecution` 을 노출해
  화면 브리핑과 **같은 함수**를 지나게 하고 예정 세션에 실어 보낸다(실측 5세션 ≈ 1.2k 토큰).
- **#796 (PR#798) 장기기억 중복이 활성 칸을 잡아먹던 문제.** 이슈 전제가 틀렸다 — 서사 기억
  (`coach_memory_items`)은 **204건 살아 있었다**(구조화 필드만 보고 "전부 비었다"고 판단했던 것).
  실제 병은 **204건 중 85건(42%)이 같은 사실의 재서술**, 한 사실은 **13번**. 활성 기억은 6칸뿐이라
  중복이 다른 기억을 밀어냈다. `_shared/memoryDedupe.ts` 신설(토큰 집합 유사도+활용형 정규화+부정
  가드), 쓰기는 **코퍼스 전체** 대조, 읽기는 선정 뒤 접기(자가 치유). 사용자가 직접 SQL 로
  **204 → 118건 정리 완료**(계획과 정확히 일치 검증).
- **보안 회차 (PR#799·#800).** Supabase advisor 6건 중 **실제 위험 1건**:
  `consume_edge_function_rate_limit`(security definer)이 **anon 키로 호출 가능** →
  남의 user_id 로 반복 호출하면 그 사용자 코칭이 차단됐다. ⚠ 원 마이그레이션의
  `revoke all ... from public` 은 **안 통한다** — Supabase 가 `alter default privileges` 로
  anon/authenticated 에 **명시 grant** 를 붙이므로 `revoke execute ... from anon, authenticated` 필요.
  또 대시보드 비번 정책(8자+4종)을 올린 뒤 앱이 길이만 봐서 **영어 원문이 노출**되던 것도 수정.

### 이 영역 함정 (오늘 값비싸게 배운 것)
- **결정론 주입은 "값"만이 아니라 "위치"까지다.** 올바른 execution 을 같은 요청에 실어도 코치가
  옛 숫자를 반복했다 — 23k+ JSON 안 한 필드는 **자기 과거 답변의 확신을 못 이긴다.** 컨텍스트 JSON
  **뒤에** 꼬리표로 다시 세우고 "이전 숫자는 폐기"라고 못박은 뒤에야 교정됐다(`_shared/executionGuideTail.ts`).
- **그 꼬리표가 한 번 사라졌다** — `context.upcomingSchedule` 은 `structuredCoachContext` 로 가려진다.
  같은 질문을 "다시 물어볼게"로 감싸자 분류가 갈려 통째로 없어졌다. **원본**
  (`scheduleProposalGate.upcomingTargets`)을 읽어야 한다. 같은 함정에 세션 액션 4종이 폐기됐던 전례가
  코드 주석에 이미 있었다("문구 분류에 데이터 가용성을 걸지 않는다"). 테스트로 잠금.
- **검증은 오염된 스레드에서.** 틀린 답변이 쌓인 그 방에서 교정되는지가 판정 기준이다. 깨끗한 방은
  통과해도 의미 없다.
- **프롬프트 지침으로 중복을 막을 수 없다.** 모델은 이번 턴에 오른 것(≤16건)만 안다. "이미 있으면 넣지
  마라"는 지켜질 수 없는 지시였다 — 코드가 코퍼스 전체로 판정한다.
- **임계값은 실측으로.** 0.6→149그룹 / 0.4→119그룹, 새로 병합되는 0.40~0.44 구간 42쌍을 표본 검사해
  0.4 채택. 활용형은 **뒤따르는 한글까지 먹어야** 한다("더울/편한지" 누수를 배포 후 라이브에서 발견).
- **코퍼스를 컨텍스트에 담되 프롬프트엔 싣지 말 것** — `NON_PROMPT_CONTEXT_KEYS` 로 직렬화에서 빼고
  테스트로 잠근다(입력이 비용의 95%).

### 오후 추가 출하 (사용자 질문에서 파생 — 전부 라이브 검증)
- **자유 텍스트 노트 마지막 둘 제거 (PR#803).** 훈련 기준의 `longRunStrategy`·`currentVolumeNote`.
  볼륨은 매 턴 실시간 계산되는데 손으로 적은 숫자가 프롬프트에 같이 실렸고(기본값 "최근 반달 114km"는
  **개발용 예시 데이터**였다), 장거리 전략은 루틴 정보라 주기화 플랜과 겹쳤다. **읽는 쪽은 원래 없었고
  쓰는 쪽만 둘**(사용자 편집 + coach patch)이라 어긋날 여지만 컸다. 이 두 칸이 9/6 유실 사고에서
  날아간 바로 그 키다 — **비어 있던 이유가 "안 쓴 것"이 아니라 "지워진 것"이었는데, 채우는 게 아니라
  없애는 게 정답이었다.** 훈련 기준 패널은 이제 읽기 화면(저장 섹션에서 빠짐).
- **"기억해줘"를 코드가 보장 (PR#804).** 명시 요청에 **보장이 없었다** — 코치가 담을지는 소프트 지침이고,
  담아도 `looksLikeDurableMemory` 키워드 화이트리스트에 안 걸리면 조용히 폐기되는데 코치는
  "기억해둘게요"라고 답했다(앱이 거짓말). ① 요청 감지를 코드가(`_shared/explicitMemoryRequest.ts`,
  **질문 제외** — "뭘 기억해?"를 요청으로 읽으면 물어볼 때마다 화이트리스트가 열린다) ② 요청 턴이면
  화이트리스트 우회(중복·최소길이는 유지) ③ 결과를 코드가 알림(`_shared/memoryIntake.ts`) + 프롬프트에
  "저장을 단정하지 마라". 관측 `data_query_log.memoryIntake{explicit,stored,skipped}`.
  ⚠ 문장 분리 시 **물음표를 남겨야** 한다("어떻게 기억해?"가 요청으로 읽혔다).
- **대화 스레드 페이지네이션 (PR#805).** 전역 163턴 중 **67턴만** 화면에 도달했다(스코프 없이 최신 80건
  조회 → 세션 228건이 예산 잠식, 8/22 이전은 볼 방법 없음, 에러 없음). 받은 건 전부 렌더해 39,099px.
  스코프를 **쿼리에** 걸고 20턴씩 커서로 읽는다 → 20턴·13,519px, "더 보기"로 163턴 전부 도달.
  ⚠ 함정: "더 보기"가 바닥으로 튀었다 — **새 턴 추종 감시자가 옛 대화를 붙일 때도 발동**한다.
  그 구간만 끄고, 감시자 nextTick 이 지난 뒤에 푼다.

- **#806 기억 탭 노출·삭제 (PR#808, 완료).** "장기 메모 0개"인데 실제 121건이었다 — 화면은
  `aiNotes` 만 세고 `coach_memory_items` 를 **웹에서 조회조차 안 했다.** 이 화면이 #796 의 전제를
  잘못 잡게 만든 장본인이다. 목록(20건 커서 페이지)+항목 삭제 추가. ⚠ `coach_memory_items` 에
  **delete 정책이 없었다** — 지금까지 삭제 경로는 대화 턴 삭제의 FK cascade 뿐(cascade 는 RLS 를 안 탄다).
  ⚠ 처음엔 패널 열 때만 읽게 했더니 바깥 목록이 "0개"로 남았다(고치려던 거짓말 그대로) → 탭 진입 시 로드.

- **추세 톤 배지 줄바꿈 (PR#811).** 폰에서 "주의"가 `주 / 의` 로 접혔다. `.trend-confidence` 가
  `.trend-overall-header`(space-between flex)의 아이템인데 **줄어들 수 있게 열려 있어서**, 폭이
  모자라면 `min-width:0` 인 제목이 아니라 **배지가 먼저 찌그러졌다.** `flex-shrink:0` + `nowrap`.
  375·320px 라이브 확인(긴 라벨 "신뢰도: 보통" 포함). 교훈: space-between 양끝 중 **안 줄어야 할 쪽**을
  명시하지 않으면 짧은 쪽이 희생된다.

### 남은 것
- **#809 장기기억 160건 조회 창** — 넘으면 오래된 기억이 **중복 판정과 소환에서 동시에 조용히 빠진다**
  (#798 에서 막은 중복이 그 구간부터 재발). 실측 하루 1.1건·중복 차단 후 주 2~3건 → **4~5개월 뒤** 도달.
  기운 쪽은 "대조 전용 경량 전량 조회"(자동 삭제는 사용자 모르게 기억을 지운다). **150건 근처에서 착수.**
- **구조화 층은 여전히 0**(runnerIdentity·coachBeliefs·compliancePatterns·sessionGuides). **새 쓰기
  경로를 만들지 않았다** — 서사 기억 + 라이브 산출(coachAdaptiveProgress·실행 지침·tempoCeiling)로 이미
  덮이는 층이라 두 번째 진실이 된다. 근거는 SSOT §장기기억에 적재. `reference_count` 는 쓰이지도
  읽히지도 않는 죽은 컬럼.
- **#801 유출 비밀번호 차단** — Pro 전용이라 보류. **Pro·유료화·상용화 화제가 나오면 먼저 꺼낸다**
  (사용자 명시 요청, 메모리 `deferred-until-pro-or-launch`).
- 온보딩 "처방 확인" 단계는 타입체크만 — 온보딩 완료 계정이라 라이브 미확인.
- `vector` 확장 public 스키마: 실사용 중(임베딩 컬럼+ivfflat)이라 이동 안 함. 그 임베딩 컬럼은
  **만들어만 두고 미사용** — 훈련 지식 검색 붙일 때의 자리.
- 이전 세션 이월: #711 워치 잔여(구간 발화·HR warning·`WatchSpeech.swift:55`)는 개인 맥+집 와이파이 필요.
