...
## ⭐ 현재 작업 — 에픽 #813 "코치는 나를 안다" 완료 (2026-09-16)

한 줄 요약: **데이터를 가진 우리가, 데이터 없는 ChatGPT 에게 졌다** — 그 원인을 구조로 고쳤다.

계기: 사용자가 같은 날 같은 주제(족저근막 통증)를 ChatGPT 와 우리 코치 양쪽에 묻고 **ChatGPT 에
만족·우리에 실망**했다. ChatGPT 는 몰라서 물어본 뒤 "4.6km/39분이면 강도 낮은 편"이라 판정했고,
우리는 DB 에 4.61km/39분을 두고도 "한 주에 1회 정도**라면**"이라고 추측체로 답했다.

에픽 #813 · 하위 #814·#815·#816·#817 + 파생 #818. PR #819·#820 머지·배포·라이브 검증 완료.

### 고친 것
- **#814 개인 사실 상시 탑재.** `detectUserNoteRunRelevance` 가 general 로 오분류하면 부상·플랜 등
  ~25개 필드가 통째로 null 이 됐다("어제 **뛴** 세션데이터" — 어형 목록에 `뛴` 이 없어 샘).
  **같은 게이트의 3번째 재발**(#701·#642). 키워드를 더 넣는 대신 구조를 바꿨다 — **문구 분류는 말투만
  정하고 개인 사실의 가용성은 분류와 무관**. 선례 3개(raceBenchmark·scheduleProposalGate·coachThread)를
  원칙으로 승격. ⚠ **지침층도 같이 고쳐야 했다** — 시스템 프롬프트가 "activeInjuryItem 이 없다고 보고
  답한다"고 못박고 있었다(#697 이 한쪽 분기만 고친 자리).
- **#818 queryRuns 모순 필터.** 모델이 `date = 9/15` + `date = 9/16` 을 보냈고 AND 라 **논리적으로 항상
  0건**. 코치가 "어제 기록이 없다"고 거짓을 말했다. 같은 필드 eq 는 **집합(OR)** 으로 읽는다.
  덤으로 앱 137 / 코치 136(=136.77) 불일치 → 심박·케이던스·칼로리 정수 확정.
- **#815 통증 판정 = 기준선 복귀.** 고정 밴드(0~2/5)는 기준선이 3인 사람에게 영원한 빨간불이라 부하
  적절성을 판정할 수 없다. Silbernagel 2007 Pain-Monitoring Model 적재.
- **#816 예후 = 개월 단위.** "조용해지면"만 반복하면 언제까지인지 알 수 없다. 족저근막증 RCT n=200.
- **#817 죽어 있던 안전 기준.** `noImprovementWeeks` 는 타입·판정·단위테스트가 다 있는데 **채우는 곳이
  없어** KB §4 "6주 무호전 → 의뢰"가 한 번도 켜질 수 없었다. 배선 + 질환별 임계(부하성 건/근막 12주).

### 이 영역 함정
- **데이터층만 고치면 지침층이 덮어쓴다.** 도구(queryRuns)와 데이터를 손에 쥐고도 "개인화 단락 생략"
  지침 때문에 안 썼다. 프롬프트 문자열은 타입·테스트가 안 걸리니 **소스 가드**로 잠근다.
- **단위테스트 통과 ≠ 배선됨.** 판정 함수를 직접 부르는 테스트는 생산자 부재를 못 잡는다.
- **Codex 5라운드 6건 전부 "안전을 넓히는" 실수**였다(top-N 을 확정으로 읽기·축소목록으로 안전판정·
  정규화 필드 유실·시점 불일치). 자체 검토로는 하나도 못 잡았다. 완화는 `every()` 로.
- **#817 의 원 가설은 틀렸다** — 감별 침묵은 프로브 게이트가 아니라 #814 탓이었다. 라이브 검증으로
  전제가 뒤집혔고, 진짜 결함(죽은 안전 기준)은 그 과정에서 찾았다.
- ⚠ **Vite dev 캐시 stale** 로 런타임 `not defined` 가 났다(타입체크·1399 테스트·빌드는 전부 통과).
  `rm -rf node_modules/.vite` 후 해소. 프로덕션은 정적 import 라 무관.

### 잔여
- `loadTolerantTendinopathy` 는 4개 가설(족저근막·아킬레스·둔근·PHT)에만 저작. 새 가설 추가 시 함께 판단
  (미설정 = 보수적 6주 · 통증 허용 미적용).
- `.claude/launch.json` 이 이번에 추적 대상으로 들어갔다(dev 서버 프리뷰 설정).

## (이전) ⭐ 현재 작업 — 감별 §5 정밀화 2건 출하: 답변 likelihood 그라데이션(#522) + monitoring 노출 게이트(#525) (2026-06-25 후반, PR #526 머지·트리검증 IDENTICAL)
