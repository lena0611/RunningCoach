...
## 2026-08-06 - (사업) 유료화 가격·경계·결제 경로 확정 (구현 홀딩)

요청당 원가를 **실측**한 뒤 가격과 유료 경계를 정했다. 결정만 확정하고 **구현은 홀딩**(사용자 지시).

- **원가 실측(계측 상시화 PR#660)**: `stream_options.include_usage` 로 실제 usage 를 받는다. 모델은 `gpt-5.4-mini-2026-03-17`, 단가 입력 $0.75 / 캐시입력 $0.075 / 출력 $4.50 per 1M.
  - 짧은 대화턴 prompt 23,472 / completion 163 → **약 25원**
  - 긴 설명(4문단) prompt 23,262 / completion 535 → **약 26원**
  - 데이터 질문(2라운드) prompt 41,851+41,993(2R 캐시적중 41,600) → **약 50원**
- **구조적 발견 2가지가 가격 설계를 지배한다**: ① 원가는 출력이 아니라 **입력**이 95% — 답변 길이를 조여도 원가는 안 줄고 **컨텍스트 크기가 곧 원가**다. ② **캐시 적중률 16%**(정적 프리픽스 3.7k만) — 프롬프트를 "안 바뀌는 것 먼저, 바뀌는 것 나중"으로 재배치하면 캐시 단가(입력의 1/10)가 먹어 원가가 1/3(월 2,250원→900원)로 내려간다. 2R에서 41.6k가 캐시된 것이 근거.
- **결정 1 — 가격: 월 6,900원**(연 49,000원 병행 여지). 원가 2,250원(하루 3턴) 대비 마진 67%, 캐시 최적화 후 87%. Strava 8,900원보다 낮게 두어 진입 저항을 줄인다.
- **결정 2 — 유료 경계는 "코치와 대화하는 양"에만 건다.** 무료 주 3회 → 이후 구독. 스케줄·부상·레이싱·업적·워치·기록·추세·날씨는 **전부 무료**. 근거: (a) **무료가 유료의 원료** — 기록이 쌓여야 "과거의 나를 아는 코치"가 값을 한다. 결정론 기능을 막으면 데이터가 안 쌓이고 유료 가치가 함께 죽는다. (b) **안전은 유료화하지 않는다** — 부상 redFlag·복귀 게이트를 돈으로 막는 구조는 신뢰·평판 양쪽에서 손해. (c) 원가가 붙는 건 LLM 호출뿐이고 나머지는 원가 0이라 과금 근거도 없다.
- **결정 3 — 크레딧/충전제 배제.** 원가 구조에는 맞지만 "대화할 때마다 돈 세는" 경험이 앱 북극성(잦은 앱↔코치 소통, [[coach-proactive-communication-vision]])과 정면 충돌한다. 주 3회 리셋은 부수적으로 매주 돌아올 이유가 되고, 무료 유저 원가를 월 250원으로 고정한다.
- **결정 4 — 결제는 Apple 유료 개발자 계정 + 앱스토어 IAP 경로로 간다**(사용자 결정). 웹 결제(포트원/토스, 수수료 3% 내외)가 즉시 가능하고 수수료도 낮지만 채택하지 않았다. **현재 Apple 유료 계정이 없어 이 경로 전체가 대기 상태**이며, 계정 확보가 선행 조건이다. IAP 수수료 15~30%는 위 마진 계산에 아직 반영되지 않았다 → 계정 확보 시 재계산 필요.
- **선행 후보(결제와 독립, 미착수)**: 캐시 최적화(프롬프트 프리픽스 재배치)는 결제 경로와 무관하게 원가를 1/3로 낮추므로 유료화 전에 하는 편이 유리하다. 무료 상한(주 3회) 게이트도 결제 없이 원가 방어만으로 먼저 넣을 수 있다. 둘 다 **사용자 승인 전까지 착수하지 않는다**.
- 기존 방어선: `COACH_RUN_RATE_LIMIT_PER_HOUR` 기본 12회/시간, `APP_SECURITY_MODE=allowlist`(공개 출시 전). [[coach-data-question-value-and-query-tool]] [[devicecheck-security-deferred]]
...
- 액션 순수로직: `reschedule.ts`(proposeReschedule=**처방 보존 이동**[재파생 금지 #405]·주넘기 소프트경고 / proposeMoveToToday / proposeSwap=충돌해결) + `weeklyTriage.ts`(weeklyHardLoadGuard=harder 소프트경고 / weekEndTriage=키세션 하나 살리고 나머지 release). 충돌=**스왑**(더블/N세션은 후속 분리 — 현 모델 하루 1세션, minGap 5h 하한 동반 `DOUBLE_MIN_GAP_HOURS`).
...
## 2026-06-22 - 더블 minGap Phase 4 v1 = 웹 동적 안내(네이티브 하드 차단은 분리 보류)
- 배경: #455 더블 웹 기반(Phase 1~3) 머지 후, 잠겨 있던 "네이티브 minGap=하드 차단(런타임 가드)"을 Phase 4 로 구현하려다 코드 그라운딩에서 전제가 깨짐을 확인.
- 발견(핵심): **웹에는 일반 훈련 세션의 '시작' 이벤트가 없다.** 훈련 런은 전부 워치로 뛰고 HealthKit 으로 *사후 import*된다. 인앱 라이브 트래킹(`RacePage`)은 셀프레이스 전용이라 더블 PM(이지)과 무관하고, 브리핑 카드 액션도 의향 확인일 뿐 시작이 아니다. → 원래 그릴에서 고른 "enterRace 시작 게이트"는 **걸리는 지점이 없다**.
- 있는 것: AM 세션의 매칭 런(`session.runId`)에서 **실제 종료시각 `endAt`** 을 얻을 수 있어, "오전 종료 기준 오후 권장 시작 시각"을 정확히 계산·안내할 수 있다.
- 결정(사용자 합의 — AskUserQuestion 2026-06-22, "동적 안내 중심" 선택): **v1 = 웹 선제 동적 안내.** `evaluateDoubleGap(amEndAt, at)`로 오전 종료 +5h(하한)·+7h(최적)와 `classifyDoubleGap` 판정을 더블 패널·추가 시트 gap 바에 색으로 표시. **인터랙티브 차단·강한 확인은 두지 않는다**(막을 시작 이벤트가 없어 오발동만 만든다). 실제 시작을 관측하는 **하드 런타임 가드는 네이티브 후속(#462)** 으로 분리(웹↔iOS 원자 동시 변경, 수동 Xcode 검증).
- SSOT 정합: `running-coaching-standards.md §같은 날 2세션` minGap 항목을 "두 단계(v1 웹 안내 / 네이티브 후속 하드차단)"로 수정 + "구현 운영값(Phase 4 v1)" 추가. 잠긴 "네이티브 하드 차단" 단정 표현 교정.
- 코치 정합(#전문코치리뷰 관점): 적격 게이트=하드 차단 유지(초보 차단), minGap 타이밍=안내(실제 코치도 성인의 런을 물리적으로 막진 못함). 강한 확인은 시작을 관측하는 네이티브에서 자연스러움.
- 적용 범위: `src/shared/lib/coaching/doubleSession.ts`(+`evaluateDoubleGap`·test), `src/pages/dashboard/{SessionDoublePanel,DoublesAddSheet,DashboardPage}.vue`, `.harness/project/running-coaching-standards.md`. 후속(#462 native): 슬롯 실시각화 + LiveRunTracker 시작 가드 + 종료시각 영속 + 브리지 계약.

## 2026-06-22 - 더블 minGap #462 = 웹 단독 강한 확인(네이티브 하드가드 불필요로 재정의)
- 배경: #462 "네이티브 minGap 하드가드(iOS 런타임)" 착수 전 코드 그라운딩(5개 병렬 리더: 이슈·LiveRunTracker·RunContextWebView·웹 브리지·코치 SSOT).
- 발견(핵심): **네이티브 단계의 전제("실제 시작을 관측")도 깨졌다.** 인앱에서 관측 가능한 '시작'은 라이브 트래킹뿐이고 그 유일 소비처는 `RacePage`(셀프레이스, #229)다. 더블 PM은 강제 이지/회복(`forcePmEasyType`)이라 셀프레이스를 안 타고 보통 워치로 뛰어 HealthKit import → 네이티브도 가로챌 시작점이 사실상 없다. 즉 v1에서 "네이티브로 넘긴" 근거가 네이티브에서도 성립하지 않는다.
- 결정(사용자 합의 — AskUserQuestion 2026-06-22): **범위 = 인앱 라이브 시작에만 / 강도 = 강한 확인 + 오버라이드.** 귀결: "인앱 라이브 시작"은 **웹(`RacePage`→`useLiveRun.start`)이 직접 일으키는 이벤트**이므로 강한 확인은 웹에서 띄우는 게 정확·단순하다 → **#462는 네이티브/브리지/Xcode 없이 웹 단독 변경으로 재정의**. v1이 "확인을 두지 않는다"고 한 건 워치 import엔 시작 이벤트가 없어 오발동이기 때문인데, 인앱 라이브 시작 지점엔 그 문제가 없다(사용자가 실제로 누른 순간에만 발동).
- 구현: `RacePage.beginCountdown`에서 `runStore.sortedRuns`의 최근 종료 런 `endAt` 기준 `evaluateDoubleGap`(at=now)이 `blocked`(<5h)면 확인 오버레이("회복할 시간이에요" + 권장 시작 시각 + '조금 더 쉬기'/'그래도 시작") → 오버라이드 후 카운트다운 재진입. **5h 창이 곧 '같은 날 둘째'를 의미해 날짜키/타임존 매칭 불필요**(20h 전 런이면 verdict=ok라 안 뜸). 공유 confirm 컴포넌트가 없어 기존 `.race-countdown` Teleport/fade·`.race-btn` 패턴 재사용.
- 코치 정합(SSOT 선독 + #전문코치리뷰 관점): minGap=안내 + 강한 확인(물리 차단 아님), 적격 게이트만 하드 차단(불변). 거짓 생리 컷오프(코르티솔 등) 단정 금지·권고 톤. `running-coaching-standards.md §같은 날 2세션`의 "네이티브 후속 하드차단" 표현 3곳을 최종 형태로 교정.
- 검증: harness:check(test + vue-tsc build) 통과. 후속(저위험): 확인 오버레이 실렌더 스팟체크(셀프레이스로 같은 날 둘째 시작 시 — #455 더블 UI와 동일 바). 워치 임포트 런은 여전히 안내만(인앱 시작 이벤트 없음).
- 적용 범위: `src/pages/race/RacePage.vue`, `src/shared/lib/coaching/doubleSession.ts`(주석 정합), `.harness/project/running-coaching-standards.md`, 본 로그.

## 2026-07-02 - AI 코칭 스트림 완료 기준: 리포트 저장과 보조 메모리 저장 분리 (#563)
- 배경: 앱에서 AI 코칭 응답이 스트리밍 중 보이다가 완료 시점에 사라지는 문제가 있었다. 최초 조치는 UI가 실패 시 partial text를 보존하는 방어였지만, 사용자가 "저장 실패의 근본 이유"를 요구해 저장/완료 경계를 재검토했다.
- 결정: `coach_reports` 저장 성공을 사용자 응답 완료의 핵심 기준으로 둔다. 이후 `training_memory.upsert`와 `coach_memory_items.insert`는 장기 개인화 보조 저장이므로 실패해도 이미 생성·저장된 리포트의 `done` SSE를 막지 않는다.
- 구현 기준: `coach_reports.insert` 실패는 fatal stage(`coach_reports.insert`)로 남기고, 보조 저장 실패는 `persistenceWarnings`와 Edge Function warning log로 남긴다. 스트림 fatal error도 `stage`를 SSE error payload와 function log에 포함해 다음 장애가 "저장 실패"로 뭉개지지 않게 한다.
- 클라이언트 결정: 스트림 EOF 직후 `TextDecoder` 잔여 버퍼를 flush하고 마지막 SSE 조각에 종결자가 없으면 한 번 더 파싱한다. 정상 `done`이 마지막 조각에 걸린 경우를 완료 실패로 오판하지 않기 위함이다.
- 포기한 대안: 보조 저장을 모두 성공시켜야 리포트를 완료로 보는 기존 결합 구조를 유지하지 않는다. `coach_memory_items` 중복/일시 실패 같은 비핵심 저장 문제가 사용자 답변 소실로 확대되기 때문이다.
- 검증: targeted Vitest(`coachRepository.stream`, `coachStreamFailure`), `npm run supabase:functions:check`, `npm run build`, `npm run harness:check` 통과.

## 2026-09-08 - 코칭 실행 지침·장기기억의 "두 번째 진실" 제거 (#767 후속·#795·#796)
- 배경: 사용자 관찰 하나에서 출발했다 — "코치가 내린 이지스트라이드가 앱 브리핑과 다르다". 기억 탭의 `adaptiveTrainingProfile.prescriptionTemplates`가 `워밍업 10분 → 20초 가속/1분40초 회복 ×8 → 쿨다운 15분`(이지 본런 없는 인터벌)을 들고 있었고, 이는 SSOT §Easy + Strides(본런 끝·15~20초·사이 60~90초 걷기·반복수는 단계/VDOT/부상 산출)와 정면 충돌한다. SSOT §14는 이미 "고정값(8회 등)이 아니다"로 그 템플릿을 부정하고 있었는데, 프롬프트는 동시에 "다음 훈련 제안 시 이 템플릿 구조를 우선 보라"고 지시하고 있었다(같은 프롬프트 안 상반된 두 지시).
- 실측 근거: 사용자 계정의 템플릿 8종이 온보딩 시드에서 **한 글자도 바뀌지 않았다**(id까지 코드 기본값과 1:1). "누적 데이터로 AI가 조정하는 기준"이라는 전제 자체가 성립한 적이 없다.
- 결정 1 (제거): 세션 실행 지침을 **코칭 기억에 저장하지 않는다.** 정본은 주기화 플랜(`training_schedule`) + `sessionBriefing` 하나다. 메모리에 처방을 복사하면 두 번째 진실이 되어 어긋난다(`weeklyPattern` 제거와 같은 구조). 레거시 자유 텍스트(`knownIssues`·`runningStyle`·`heatStrategy`)도 후속 구조화 필드(injuryItems·runnerIdentity·날씨 코칭 SSOT)로 대체돼 제거. `aiNotes`는 코치가 쓰는 칸이므로 남기되 **읽기 전용**.
- 결정 2 (주입): 코치에게 `upcomingSchedule[].execution`으로 **화면과 같은 함수**(`buildSessionExecution`)의 산출물을 실어 보낸다. 제거만 하면 코치에게 실행 지침 정본이 없어 기억으로 메꾼다(실측: 제거 후에도 옛 숫자를 4턴 연속 반복하며 "앱 기준으로는"이라는 틀린 출처까지 붙였다).
- 핵심 교훈 (결정론 주입의 확장): **값을 주는 것만으로 부족하고 위치까지 코드가 정해야 한다.** 올바른 execution이 같은 요청에 실려 있어도 23k+ JSON 안의 한 필드는 모델 자신의 과거 답변을 못 이겼다. 컨텍스트 JSON **뒤에** 꼬리표로 다시 세우고 "이전 답변의 다른 숫자는 폐기"라고 못박은 뒤에야 교정됐다(`_shared/executionGuideTail.ts`). 프롬프트 지침 추가만으로는 실패했다.
- 핵심 함정 (재발): 그 꼬리표를 `context.upcomingSchedule`에서 읽자 `structuredCoachContext` 축약 모드에서 통째로 사라졌다(같은 질문을 "다시 물어볼게"로 감싸자 분류가 갈림). **원본**(`scheduleProposalGate.upcomingTargets`)을 읽어야 한다 — 세션 액션 4종이 같은 함정으로 구조적으로 폐기됐던 전례가 코드 주석에 이미 있었다("문구 분류에 데이터 가용성을 걸지 않는다"). 테스트로 잠금.
- 결정 3 (기억 중복): #796의 최초 전제("장기 기억이 전부 비었다")는 **틀렸다** — 구조화 필드만 본 결과였고, 서사 기억 `coach_memory_items`는 204건 살아 있었다. 실제 병은 **204건 중 85건(42%)이 같은 사실의 재서술**(한 사실 13회)이고, 활성 기억이 6칸뿐이라 중복이 다른 기억을 밀어낸 것. 중복 판정을 프롬프트 지침이 아니라 코드로 옮긴다(`_shared/memoryDedupe.ts`): 쓰기는 **코퍼스 전체** 대조(기존엔 이번 턴 프롬프트에 오른 ≤16건만 봤다 — 모델은 방금 본 것만 아니까 "이미 있으면 넣지 마라"는 지켜질 수 없는 지시였다), 읽기는 선정 뒤 근사 중복 접기(저장 행은 안 지우는 자가 치유). 임계값은 실측 코퍼스로 결정(0.6→149그룹 / 0.4→119그룹, 0.40~0.44 구간 42쌍 표본 검사 후 0.4). 코퍼스는 `NON_PROMPT_CONTEXT_KEYS`로 프롬프트에서 제외(입력이 비용의 95%).
- 구조화 층 판단 (안 만든 것): `runnerIdentity`·`coachBeliefs`·`compliancePatterns`·`sessionGuides`는 전부 0이지만 **새 쓰기 경로를 만들지 않았다.** 서사 기억 + 라이브 산출(coachAdaptiveProgress·실행 지침·tempoCeiling)로 이미 덮이는 층이라 세 번째 진실이 된다. 근거를 SSOT §장기기억에 적재하고, 새 경로를 만들기 전 확인할 것을 명문화.
- 검증 원칙: **오염된 스레드에서 검증한다.** 틀린 답변이 4번 쌓인 그 방에서 교정되는지가 판정 기준이고, 깨끗한 방은 통과해도 의미 없다. 실제로 1차 배포는 그 방에서 실패해 원인을 찾았다.
- 데이터 정리: 사용자가 직접 SQL로 중복 86행 삭제(204→118). 삭제 계획은 배포된 판정 모듈로 산출하고(즉석 재구현 금지), 삭제 전 전량 백업 + 주제 손실 대조(목표 9/9·휴식 5/5·동반주 2/2 전량 보존) 후 실행. 결과가 계획과 정확히 일치함을 사후 검증.
- 적용 범위: `src/entities/training-memory/model.ts`, `src/pages/memory/MemoryPage.vue`, `src/pages/onboarding/OnboardingFlow.vue`, `src/shared/lib/coaching/{initialWeeklyPattern,sessionBriefing}.ts`, `src/features/coach-session/CoachSessionOverlay.vue`, `src/shared/api/coachRepository.ts`, `supabase/functions/coach-run/index.ts`, `supabase/functions/_shared/{executionGuideTail,memoryDedupe}.ts`, `.harness/project/{running-coaching-standards,domain-rules,architecture-rules,navigation-information-architecture,ui-system-contract}.md`.

## 2026-09-08 - Supabase 보안 조언 회차: 실제 취약점 1건과 "안 고치는 것"의 근거 (#799·#800·#801)
- 배경: Supabase security advisor 6건(INFO 2 + WARN 4)을 사용자가 공유. 전부 고치는 대신 **누가 그 객체를 쓰는지**로 실제 위험을 갈랐다.
- 실제 취약점 (조치): `consume_edge_function_rate_limit`(security definer)이 `anon`·`authenticated` 역할로 REST RPC 호출 가능했다. anon 키는 웹 번들에 있으므로 사실상 공개. 남의 `user_id`로 반복 호출하면 **그 사용자의 코칭 호출 제한을 대신 태워** 남은 시간 동안 AI 코칭을 차단할 수 있었다(자기 제한 우회는 불가 — `window_start`를 coach-run이 서버에서 계산).
- 핵심 함정: 원 마이그레이션(202606020001)이 이미 `revoke all ... from public`을 하고 있었으나 **그걸로는 안 막힌다.** Supabase가 `alter default privileges in schema public grant execute on functions to anon, authenticated, service_role`를 걸어두므로 함수 생성 시점에 두 역할에 **명시 grant**가 붙고, PUBLIC 회수는 그걸 건드리지 않는다. `revoke execute ... from anon, authenticated`가 필요하다.
- 안 고친 것과 근거: (1) `app_sessions`·`edge_function_rate_limits`의 "RLS 켜짐 + 정책 없음"은 **정답**이다(= 클라이언트 전면 차단, service_role 전용). 정책을 추가하는 게 오히려 사고이므로 의도를 테이블 주석으로 명문화. (2) `vector` 확장 public 스키마는 실사용 중(`training_knowledge_chunks.embedding vector(1536)` + ivfflat)이라 이동 시 타입 참조·search_path 리스크가 크고, 노출 위험이 아니라 네임스페이스 위생 문제라 유지.
- 파생 버그(우리가 만든 것): 대시보드 Password requirements를 "소문자·대문자·숫자·기호"로 올린 뒤 앱은 길이만 검사해, 서버 거부 시 알파벳이 나열된 영어 원문이 사용자에게 그대로 노출됐다. `passwordPolicyIssue`로 서버 규칙과 일치시키고 **빠진 종류만 지목**하도록 수정(로그인에는 미적용 — 옛 규칙 계정이 잠기면 안 된다).
- 보류 + 트리거: 유출 비밀번호 차단(HaveIBeenPwned)은 **Pro 전용**이라 무료 플랜에서 거부됨. 사용자 1명 앱에서 실익이 낮아 이슈 #801로 보류하되, **Pro·유료화·상용화 화제가 나오면 에이전트가 먼저 꺼낸다**(사용자 명시 요청, 장기 메모리 `deferred-until-pro-or-launch`에 트리거 저장).
- 검증: `revoke` 후 anon 키 호출 401 / 로그인 토큰 호출 403 / 앱 AI 코칭 1회 정상(service_role 경로 무영향). harness:check 통과.

## 2026-09-08 (오후) - 사용자 요청 기억의 보장 주체와 대화 스레드 조회 계약 (#803·#804·#805)
- 배경: 세션 마무리 중 사용자가 던진 질문 세 개가 각각 결함을 드러냈다 — "장거리 전략·볼륨 노트는 누가 채우나", "명시적으로 기억해달라고 하면 반드시 들어가나", "대화는 누적형이라 스크롤이 계속 쌓이나".
- 결정 1 (자유 텍스트 노트 전면 폐지): `longRunStrategy`·`currentVolumeNote` 제거. **읽는 쪽이 아무도 없고 쓰는 쪽만 둘**(사용자 편집 + coach `trainingMemoryPatch`)이었다. 볼륨은 recent7/14/30 으로 매 턴 실시간 산출되고 롱런 전략은 주기화 플랜이 갖는데, 손으로 적은 값이 프롬프트에 함께 실려 **같은 것에 대한 숫자 두 개**가 갔다. 기본값 "최근 반달 114km 누적"은 개발용 예시라 실제 사용자 수치도 아니었다. 이로써 `TrainingMemory` 의 사람 입력 자유 텍스트는 전부 사라졌다(weeklyPattern → prescriptionTemplates → knownIssues/runningStyle/heatStrategy → longRunStrategy/currentVolumeNote). 사용자 서사·선호는 `coach_memory_items` 가, 실행 지침은 `sessionBriefing` 이, 루틴은 `training_schedule` 이 갖는다.
- 관찰(진단의 전환점): 이 두 칸은 2026-09-06 유실 사고에서 날아간 바로 그 키였다. **"비어 있다"를 "안 쓴다"로 읽으면 오진**이고, 실제로는 "지워졌다"였다. 그럼에도 결론은 "채운다"가 아니라 "없앤다"였다 — 빈 이유와 존치 여부는 별개 판단이다.
- 결정 2 (명시적 기억 요청의 보장 주체 = 코드): "기억해줘"에 대한 보장이 어디에도 없었다. 모델이 memoryItems 에 담을지는 소프트 지침이고, 담아도 `looksLikeDurableMemory` **키워드 화이트리스트**에 안 걸리면 조용히 폐기되는데 코치는 "기억해둘게요"라고 답했다. 화이트리스트는 **코치가 자기 판단으로 넣는 것**을 거르는 장치이지 사용자가 직접 부탁한 것까지 거르라고 만든 게 아니다 — 요청 턴이면 우회한다(중복 차단·최소 길이는 유지). 요청 감지도 코드가 한다(`_shared/explicitMemoryRequest.ts`), **질문은 제외**: "뭘 기억해?"를 요청으로 읽으면 사용자가 기억을 물어본 턴마다 화이트리스트가 열린다(이 세션에서 실제로 여러 번 물었다). 구현 함정: 문장 분리 시 물음표를 떼면 "어떻게 기억해?"가 요청으로 읽힌다 — 구두점을 문장에 남긴다.
- 결정 3 (저장 결과 고지 = 코드 후처리): 코치는 답을 쓰는 시점에 저장 여부를 모른다(insert 는 그 뒤). 그래서 "요청했는데 아무것도 안 남은" 경우에만 코드가 한 줄 붙인다(`_shared/memoryIntake.ts`) — 중복이면 "이미 기억하고 있다", 짧으면 이유와 재요청. 저장됐으면 아무 말도 안 붙인다. 프롬프트에는 **"기억해뒀어요처럼 단정하지 마라"**를 넣어 발화와 실제를 정합시켰다. 관측은 `data_query_log.memoryIntake{explicit,stored,skipped}` 로 남겨 "요청했는데 안 남았다"가 조회 가능하다.
- 결정 4 (대화 스레드 조회 계약): 스레드 조회에 **스코프를 쿼리에 건다**. 예전엔 스코프 없이 최신 80건을 받아 클라이언트에서 걸러, 세션 대화 228건이 예산을 잡아먹고 전역 163턴 중 **67턴만** 도달했다(8/22 이전은 앱에서 볼 방법 없음, 에러 없음 — [[silent-truncation-and-scope-in-data-reads]] 재발). 20턴씩 커서 페이지네이션으로 바꿔 163턴 전부 도달 가능하게 하고 초기 렌더를 39,099px → 13,519px 로 줄였다. 개발용 목록 화면은 `fetchRecentCoachReports`(스코프 없음)로 이름부터 갈라 오용을 막았다.
